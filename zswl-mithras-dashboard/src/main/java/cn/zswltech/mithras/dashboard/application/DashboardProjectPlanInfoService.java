...
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.ContractOrgPlanMapper;
...
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.model.ContractOrgPlan;
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.model.DashboardProjectPayInfoResult;
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.model.DashboardProjectPlanInfoQuery;
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.model.DashboardProjectPlanInfoResult;
...
@Slf4j
@Service
public class DashboardProjectPlanInfoService extends DashboardProjectService implements DashboardProjectPlanApplicationService {
    ...
    public List<DashboardProjectPlanListRSP> planList(DashboardProjectPlanListREQ req) {
        ...
        if(CollectionUtils.isNotEmpty(resultList)){
            rspList = resultList.stream().map(m -> {
                ...
                rsp.setActualPayAmount(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(m.getActualPayAmountLong()), "万元"));
                rsp.setContractAmount(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(m.getContractAmountLong()), "万元"));
                rsp.setIrr(new ValueUnitDTO(DashboardAmountUtil.toYuanWithoutSplit(m.getIrrInt().longValue()), "%"));
                rsp.setCommission(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(m.getCommissionLong()), "万元"));
                rsp.setEarnestMoney(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(m.getEarnestMoneyLong()), "万元"));
    ...
    public DashboardProjectPlanStatisticsRSP statistics(DashboardProjectPlanStatisticsREQ req) {
        ...
        if(CollectionUtils.isNotEmpty(resultList)) {
            ...
            if(planPayAmount != 0 && !Objects.equals(planPayAmount,0L)){
                ...
                rsp.setPlanPayAmount(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(planPayAmount), "万元"));
                rsp.setFinishRate(new ValueUnitDTO(String.valueOf(finishRate.multiply(new BigDecimal("100")).intValue()), "%"));
            }
            rsp.setPayAmount(new ValueUnitDTO(DashboardAmountUtil.toWanYuanWithoutSplit(payAmount), "万元"));
        }
        return rsp;
    }
