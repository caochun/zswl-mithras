...
public class RentCollectionEmailServiceImpl {
    ...
    @Resource
    private RentCollectionEmailNoticePort rentCollectionEmailNoticePort;
    @Resource
    ...
    @Resource
    private RentCollectionEmailContractContactPort rentCollectionEmailContractContactPort;
    @Resource
    ...
    @Resource
    private RentCollectionEmailBankAccountPort rentCollectionEmailBankAccountPort;
    @Resource
    private RentCollectionEmailPledgeAccountPort rentCollectionEmailPledgeAccountPort;
    ...
    public void sendEmail(RentCollectionEmailSendREQ req) {
        ...
        try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
            rentCollectionEmailNoticePort.renderHtml(os, req.getCollectionId(), req.getComment(), req.getBankId());
...
//                    .fileId(fileId)
            ...
            // 更新收款主表
            afterLeaseCollectionPort.updateEmailNoticeCount(req.getCollectionId(), 1);

            rentCollectionEmailNoticePort.send(req.getReceiverMail(), req.getTitle(), os.toString(), true);
        }
    }
    ...
    public Long genEmail(Long collectionId, String comment, Long bankId) {
        ...
        String fileName = rentCollectionEmailNoticePort.renderHtml(os, collectionId, comment, bankId);
        return afterLeaseMaterialsPort.add(IoUtil.toStream(os.toByteArray()), fileName, collectionId, MATERIALS_TYPE, TMP_BUSINESS_TYPE);
    }
...
//    @Transactional(rollbackFor = Exception.class)
    public String genEmailHtml(Long collectionId, String comment, Long bankId) {
        ...
        // 判断有没有默认的
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        rentCollectionEmailNoticePort.renderHtml(os, collectionId, comment, bankId);
        RentCollectionEmailHtmlStore htmlStore = RentCollectionEmailHtmlStore.builder()
