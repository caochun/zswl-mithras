...
public class MetricComputer4A10000396_JC014 extends AbstractMetricComputer implements SubscribeSupporter<MetricComputeEvent> {
    ...
    public void calculate(MetricComputeEvent event, RiskControlStrategy strategy) {
        // 1. 计算系统剩余逾期本金+利息
        Triple<BigDecimal, BigDecimal, BigDecimal> triple = remainingPrincipalService.overdueAmount(event.getSnapshotDate());
        //这里只取系统剩余逾期本金
        Pair<BigDecimal, BigDecimal> pair = Pair.of(triple.getRight(), triple.getLeft());

        // 2. 计算系统剩余本金+利息
        Map<Long, Pair<BigDecimal, BigDecimal>> prePrincipalInterest = remainingPrincipalService.prePrincipalInterest(event.getSnapshotDate());
        // 取出结果加总就是剩余本金+利息
