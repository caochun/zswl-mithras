...
- `npm run report:ui-domain-deps` 会把已拆出的窄入口按独立能力归类，例如 `BlackGrayHit`、`CustomerDebtRatingList`、`CustomerSingleViewRisk`、`InsurancePolicyColumns`、`ProjectReviewMeetingModal`、`RentCollectionDetail`、`RiskSourceCardCalcModal`、`TrackEventModal`、`TrackEventTask`；这些报告项代表稳定共享能力，不等同于调用方依赖完整业务域。
