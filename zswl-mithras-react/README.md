...
- `src/pages`：路由入口。当前路由 JS/TS 文件应只做 `@/components/**` 转发，不承载真实实现；样式和页面私有实现跟随真实组件目录维护。
...
- `BlackGray/WarehouseApprovalEntries.js`
- `BlackGray/WarehouseMainTaskEntries.js`
...
- `Financial/FundDetailRepaymentEntries.js`
- `Financial/FundEffectEntries.js`
- `Financial/FundListComponentEntries.js`
...
- 融资管理列表页、融资生效、变更日志和差异详情通过 `src/components/Financial/FundListPageEntries.js`、`src/components/Financial/FundListComponentEntries.js`、`src/components/Financial/FundEffectEntries.js`、`src/components/Financial/FundDetailPageEntries.js`、`src/components/Financial/FundDetailRepaymentEntries.js`、`src/components/Financial/FundDetailAccountEntries.js`、`src/components/Financial/FundDetailAssetEntries.js`、`src/components/Financial/FundDetailLogEntries.js` 暴露，`src/pages/financial/fund/*` 中对应路由只保留路由壳。
...
- `npm run check:boundaries` 会扫描整个 `src` 的 JS/TS 源码和 `.less` 样式 import，禁止非 `Entries/entries` 的 `@/components/<domain>/<subpath>` 导入，禁止未登记的组件根目录直连，禁止已收敛共享业务组件的根目录直连，禁止组件域内部反向引用自身 `*Entries.js`，禁止通过 `@/pages/**` 复用页面私有代码，禁止直接引用历史 API 目录，禁止页面和组件直接引用 API interface 类型目录，禁止 `src/api` 内部跨业务域引用，并校验领域级入口已被代码使用且同步记录在 README，同时校验无引用组件候选清零、UI 跨域依赖无待语义评审项。
