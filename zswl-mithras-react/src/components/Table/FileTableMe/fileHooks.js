...
const useFile = ({
  ...
  const deleteFile = async (id, record) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        let res
        if (record.enumType === 'BASIC_INFORMATION' && _.isFunction(removeApi)) {
          res = await removeApi({ ...params, fileId: newId })
        } else {
  ...
  const onBatchDownload = async () => {
    ...
    const { ext, businessVersion, ...rest } = params
  ...
  const preview = async ({ record, id, editType = 1, idType, fileName }) => {
    ...
    if (['pdf'].includes(getFileExtension(fileName))) {
      ...
      window.open(
        `/preview/reportPreview/${newId}?editType=${editType}${versionQuery}${idTypeQuery}&forceSave=true`
      )
    }
  }
