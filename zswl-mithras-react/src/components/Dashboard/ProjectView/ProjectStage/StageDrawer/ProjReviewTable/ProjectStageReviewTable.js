...
// 评审
const ProjectStageReviewTable = ({ group, extraQueryParams }) => {
  ...
  const table = Table.useStore({
    request: async (params) => {
      const { list, sumData } = await Api.postProjectStageProjreviewList({
        ...params,
        ...extraQueryParams,
      })
      setSumData(sumData)
  ...
  return (
    <Table
      extra={
        <ExportBtn
          tableStore={table}
          businessType={'DASHBOARD_PROJECT_STAGE_REVIEW'}
          extraParams={{
            ...extraQueryParams,
          }}
        />
...
export default observer(ProjectStageReviewTable)
