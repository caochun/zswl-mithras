...
// 投放
const ProjectStagePaymentTable = ({ group, extraQueryParams }) => {
  ...
  const table = Table.useStore({
    request: async (params) => {
      const { list, sumData } = await Api.postProjectStagePaymentList({
        ...params,
        ...extraQueryParams,
      })
      setSumData(sumData)
  ...
  return (
    <Table
      extra={
        <ExportBtn
          tableStore={table}
          businessType={'DASHBOARD_PROJECT_STAGE_PAYMENT'}
          extraParams={{
            ...extraQueryParams,
          }}
        />
...
export default observer(ProjectStagePaymentTable)
