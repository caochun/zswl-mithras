...
// 付款
const ProjectStagePreparePaymentTable = ({ group, extraQueryParams }) => {
  ...
  const table = Table.useStore({
    request: async (params) => {
      const { list, sumData } = await Api.postProjectStagePreparepaymentList({
        ...params,
        ...extraQueryParams,
      })
      setSumData(sumData)
  ...
  return (
    <Table
      extra={
        <ExportBtn
          tableStore={table}
          businessType={'DASHBOARD_PROJECT_STAGE_PREPARE_PAYMENT'}
          extraParams={{
            ...extraQueryParams,
          }}
        />
...
export default observer(ProjectStagePreparePaymentTable)
