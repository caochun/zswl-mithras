...
function CustomerProfitFinancialReport({ store }) {
  ...
  const { displayDimensions = [] } = store.profitSearchBar.getParams()
    ...
    <>
      <div className={styles.searchWrap}>
        <div className={styles.searchBar}>
          ...
          >
            <Row>
              <CommonSearch searchStore={store.profitSearchBar} />
              ...
              <Item>
                <Button type={'primary'} onClick={store.profitSearchBar.search}>
                  查询
                </Button>
              </Item>
            </Row>
            ...
            <div className={styles.childWrap}>
              {Object.keys(financeList).length > 0 && (
                <div className={styles.tableCell}>
                  <Table
                                      columnsFilter={'maintain_detail_Financial_Profit_1'}
                                      onFilter={(key,val) => saveServer('maintain_detail_Financial_Profit_1',val)}
                    bordered
                    size={'small'}
              ...
              financeList.slice(1).map((obj, i) => {
                return (
                  <div className={styles.tableCell} key={i}>
                    <Table
                                        columnsFilter={'maintain_detail_Financial_Profit_2'}
                                        onFilter={(key,val) => saveServer('maintain_detail_Financial_Profit_2',val)}
                      bordered
                      size={'small'}
                      pagination={false}
                      columns={[
