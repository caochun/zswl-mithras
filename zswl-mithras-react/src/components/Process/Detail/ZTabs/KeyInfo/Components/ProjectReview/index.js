...
const ProcessDetailZTabsKeyInfoProjectReview = () => {
  const { detailData } = useFlowData()
  const { businessKey: id, businessVersion, processInstanceId } = detailData
  const [moduleData, setModuleData] = useState({})

  const getDetail = async () => {
    const baseInfo = await Api.projectReviewBaseInfoCompare({
      id,
      processInstanceId,
      businessVersion,
    })
    const { bizType } = baseInfo
    const res = await Api.projectReviewQSCompare({ projectId: id, processInstanceId, businessVersion })
...
export default observer(ProcessDetailZTabsKeyInfoProjectReview)
