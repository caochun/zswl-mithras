    ...
    return [
      {
        ...
        actions({ projName, bizType, id, projReviewStatus }) {
          return [
            {
              name: projName,
              to: `/project/price/detail/${id}?typeId=review&bizType=${bizType}`,
      ...
      {
        title: '审批状态',
        dataIndex: 'projPricingProcessStatus',
        width: 140,
        render: (item, index) => {
  ...
  return (
    <Page>
      <div className={styles.customerWrap}>
        <Table
          columnsFilter={'project_price_1'}
          onFilter={(key, val) => saveServer('project_price_1', val)}
          resizable
          store={store.table}
          selectable={{
          ...
          extra={[<PageListDown key="1" module="price" table={store.table} />]}
          searchbar={{
            labelCol: { span: 6 },
            items: [
              {
...
export default observer(ProjectPrice)
