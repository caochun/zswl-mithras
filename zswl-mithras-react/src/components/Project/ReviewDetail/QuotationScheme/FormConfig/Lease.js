...
const Lease = ({ showValue, form, detail, noTitle, compareChangeList = [], isLog }) => {
  ...
  // 不规则还款
  const onRentalCalcTypeChange = (val) => {
    if (val === 'IRREGULAR_REPAY') {
      form?.setFieldsValue({
        repayRate: undefined,
      })
    }
    ...
    >
        ...
        <FormItemContent
          ...
          formContent={
            <Form.Item
              ...
              rules={[rules.required('请输入申报授信金额'), validatorRange]}
            >
      ...
      >
        <FormItemContent
          ...
          formContent={
            <Form.Item name="creditAmountLoop" rules={[rules.required()]}>
      ...
      >
        <FormItemContent
          ...
          formContent={
            <Form.Item name="leaseMonthCount" rules={[rules.required(), validatorBigZero]}>
        ...
        <FormItemContent
          ...
          formContent={
            <Form.Item
              ...
              rules={[rules.required(), validatorAmount]}
            >
      ...
      >
        <FormItemContent
          ...
          formContent={
            <Form.Item name="repayRate" rules={[rules.required('请选择')]}>
      ...
      <Descriptions.Item
        label={
          <CommonTips fieldName={'consultingFee'}>{requiredStr('服务费/咨询费(元)')}</CommonTips>
        }
      ...
      <Descriptions.Item
        label={<StarDom name={<CommonTips fieldName={'commission'}>手续费(元）</CommonTips>} />}
      ...
      <Descriptions.Item
        label={
          <StarDom
            name={<CommonTips fieldName={'firstInstallmentInterest'}>首期利息(元）</CommonTips>}
          />
        }
      ...
      <Descriptions.Item
        label={requiredStr('支付方式')}
        labelStyle={labelRed(getDetailChange('payType'))}
      >
        <FormItemContent
          isChange={getDetailChange('payType')}
          formContent={
            <Form.Item name="payType" rules={[rules.required('请选择')]}>
      ...
      >
        <FormItemContent
          ...
          formContent={
            <Form.Item name="interestWay" rules={[rules.required('请选择')]}>
              <Select
              ...
              {({ getFieldValue, setFieldValue }) => {
                ...
                return (
                  <Form.Item name="rentalCalcType">
                    <Select
                      options={rentalCalcTypeEnum}
                      placeholder="请选择还款方式"
                      onChange={onRentalCalcTypeChange}
                    />
                  </Form.Item>
                )
              }}
          ...
          formContent={
            <Input.Group compact>
              ...
              <Form.Item
                ...
                rules={[rules.required(), validatorBigZero]}
            ...
            hasValue(getDetailValue('rateType')) && hasValue(getDetailValue('leaseRatePercent')) ? (
              <div>
                ...
                <span style={{ color: getDetailChange('leaseRatePercent') ? 'red' : '#333' }}>
                  {Number(getDetailValue('leaseRatePercent')).toFixed(2)}%
                </span>
              </div>
            ) : (
              '-'
      ...
      <Descriptions.Item
        label={requiredStr('计划起租日')}
        labelStyle={labelRed(getDetailChange('plannedStartingDate'))}
      >
        <FormItemContent
          isChange={getDetailChange('plannedStartingDate')}
          formContent={
            <Form.Item
            ...
            >
              <DatePicker
                ...
                style={{ width: '100%' }}
                placeholder="请选择计划起租日！"
              />
            </Form.Item>
          }
          value={
            getDetailValue('plannedStartingDate')
              ? moment(getDetailValue('plannedStartingDate')).format('yyyy-MM-DD')
              : undefined
          }
          showValue={showValue}
        />
