  ...
  return (
    <Page params={{ ...params, ...query }} store={store} current="在库明细">
      ...
      <Card title="在库明细" style={{ marginTop: 20 }}>
        <Table
          onFilter={(key, val) => saveServer('allQuery_groupDetail_idjs_1', val)}
          columnsFilter={'allQuery_groupDetail_idjs_1'}
          columns={listColumns}
          store={store.list}
          serial
          columnWidth={120}
      ...
      <Card title="在库下属企业" style={{ marginTop: 20 }}>
        <Table
          onFilter={(key, val) => saveServer('allQuery_groupDetail_idjs_2', val)}
          columnsFilter={'allQuery_groupDetail_idjs_2'}
