...
function RiskRelateMonitorPayment({ pullSelect }) {
  const $table = useMemo(() => {
    return new TableStore({
      request: async (params) => {
        const data = await Api.postPaymentList(params)
        return data
      },
    })
  }, [])
  ...
  return (
    <Page>
      <Table
        columnsFilter={'riskStrategy_relateMonitor_PayMent'}
        onFilter={(key, val) => saveServer('riskStrategy_relateMonitor_PayMent', val)}
        scroll={{ x: 1500 }}
        store={$table}
        editable={false}
        searchbar={{
...
export default observer(RiskRelateMonitorPayment)
