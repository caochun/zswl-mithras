...
function RiskConcentrationControlGroup() {
  const $table = useMemo(() => {
    return new TableStore({
      request: async (params) => {
        const data = await Api.postConcentrationGroupList(params)
        return data
      },
    })
  }, [])
  ...
  return (
    <Table
      columnsFilter={'concentrationControl_Group_1'}
      onFilter={(key, val) => saveServer('concentrationControl_Group_1', val)}
      columnWidth={180}
      scroll={{ x: 1600 }}
      store={$table}
      editable={false}
...
export default observer(RiskConcentrationControlGroup)
