    ...
    <>
      <Row className={styles.row}>
        ...
        <Col span={16} className={styles.right}>
          <Form.Item label="涉及自然人担保" name="B_C_8_01">
                ...
                <>
                  ...
                  <Row className={styles.row}>
                    ...
                    <Col span={16} className={styles.right}>
                      <Form.Item
                        label="是否存在相关负面消息"
                        name={[filed.name, 'B_C_8_03_01']}
                        required={isNaturalPerson}
                        rules={[{ required: isNaturalPerson }]}
                      >
                        <RadioReadOnly onlyRead={!editable} options={yesOrNo} />
                      </Form.Item>
                      <Form.Item dependencies={[['naturalPerson', filed.name, 'B_C_8_03_01']]}>
                        {({ getFieldValue }) => {
                          const required =
                            isNaturalPerson &&
                            getFieldValue(['naturalPerson', filed.name, 'B_C_8_03_01']) === '1'
                          return (
                            <Form.Item
                              label="负面信息及对担保能力的影响"
                              name={[filed.name, 'B_C_8_03_02']}
                              rules={[{ required }]}
                            >
  ...
  return (
    <>
      ...
      <InstructionsText
        ...
        radioName={[name, 'B_C_6_01_01']}
        textAreaName={[name, 'B_C_6_01_02']}
        editable={editable}
        required={required}
        listName={listName}
        triggerValue="1"
      ...
      <InstructionsText
        ...
        radioName={[name, 'B_C_6_02_01']}
        textAreaName={[name, 'B_C_6_02_02']}
        listName={listName}
        editable={editable}
        required={required}
        triggerValue="1"
      ...
      <InstructionsText
        ...
        radioName={[name, 'B_C_6_03_01']}
        textAreaName={[name, 'B_C_6_03_02']}
        editable={editable}
        required={required}
        listName={listName}
        triggerValue="1"
      ...
      <InstructionsText
        title="担保人融资渠道是否通畅"
        radioName={[name, 'B_C_6_04_01']}
        textAreaName={[name, 'B_C_6_04_02']}
        listName={listName}
        editable={editable}
        required={required}
        triggerValue="0"
      ...
      <InstructionsText
        ...
        radioName={[name, 'B_C_6_05_01']}
        textAreaName={[name, 'B_C_6_05_02']}
        listName={listName}
        editable={editable}
        required={required}
        triggerValue="1"
      ...
      <Row className={styles.row}>
        ...
        <Col span={16} className={styles.right}>
          ...
          <Form.Item dependencies={[[listName, name, 'B_C_7_01']]}>
            {({ getFieldValue }) => {
              const isRequired = getFieldValue([listName, name, 'B_C_7_01']) === '1'
              return (
                <Form.Item
                  label="具体分析"
                  name={[name, 'B_C_7_02']}
...
const GuarantorBasicInfo = observer(({ editable, required = true, namePrefix, chiName }) => {
  ...
  const B_C_5_01 = Form.useWatch('B_C_5_01', form)
  const isGuarantor = B_C_5_01 === '1'

  const B_C_8_01 = Form.useWatch('B_C_8_01', form)
  useEffect(() => {
    const naturalPerson = form.getFieldValue('naturalPerson')
    if (B_C_8_01 === '1' && naturalPerson?.length === 0) {
      form.setFieldsValue({
        naturalPerson: [{}],
      })
    }
  }, [B_C_8_01])
  const isNaturalPerson = B_C_8_01 === '1'
  ...
  return (
    <>
      ...
      <Row className={styles.row}>
        ...
        <Col span={16} className={styles.right}>
          <Form.Item label="涉及企业担保" name="B_C_5_01">
            <RadioReadOnly onlyRead={!editable} options={yesOrNo} />
          </Form.Item>
        </Col>
      </Row>
