...
function AfterLeaseGovernmentFinancial({ canEdit, store }) {
  const content = (
    <div>
      <p>金额单位</p>
      <div>
        ...
        <div className={styles.searchBar}>
          ...
          >
            <Row>
              ...
              <Item>
                {canEdit && (
                  <Button type={'primary'} onClick={store.govSearchBar.search} disabled={!canEdit}>
                    查询
                  </Button>
                )}
                {canEdit && (
        ...
        <Spin spinning={loading}>
          <div className={styles.tableWrap}>
            <div className={styles.childWrap}>
              {Object.keys(financeList).length > 0 && (
                <Table
                columnsFilter={'components_Financial_Gov_1'}
                        onFilter={(key,val) => saveServer('components_Financial_Gov_1',val)}
              ...
              financeList.slice(1).map((obj, i) => {
                return (
                  <div className={styles.childWrap} key={i}>
                    <Table
                      columnsFilter={'components_Financial_Gov_2'}
                              onFilter={(key,val) => saveServer('components_Financial_Gov_2',val)}
...
export default observer(AfterLeaseGovernmentFinancial)
