...
const ALL_COLUMNS = [
  ...
  {
    title: '还款月份',
    dataIndex: 'repayMonth',
    // requiredMark: true,allowClear={false}
    defaultFilteredValue:moment(new Date).format(`YYYY-MM`),
    search: {
      ...
      itemProps: {
        ...
        transform: (val) => {
          return {
            ['repayMonth']: val && moment(val).format(`YYYY-MM`),
          }
        },
      },
    },
  ...
  {
    title: '是否筛选本月应还金额大于0的数据',
    dataIndex: 'filterAmount',
    search: {
      element:
      <Radio.Group style={{width:'120px'}}>
  ...
  AmountColumn({ title: '实付本金（元）', dataIndex: 'actualPrincipalAmount' }),
  AmountColumn({ title: '利息（元）', dataIndex: 'interest' }),
  AmountColumn({ title: '实付利息（元）', dataIndex: 'actualInterestAmount' }),
  //   '本月支付金额', '本月收入金额',
  ...
  {
    title: '业务类型',
    dataIndex: 'financingBizType',
    matchOption: 'financingBizType',
    mode: 'multiple',
    render: (val) => App.matchOption('financingBizType', val)?.label,
  },
  //本月应还金额（元） 本月应还本金（元） 本月应还利息（元）
  AmountColumn({ title: '本月应还金额（元）', dataIndex: 'monthRepayAmount' }),
  AmountColumn({ title: '本月应还本金（元）', dataIndex: 'monthRepayPrincipal' }),
  AmountColumn({ title: '本月应还利息（元）', dataIndex: 'monthRepayInterest' }),
]
