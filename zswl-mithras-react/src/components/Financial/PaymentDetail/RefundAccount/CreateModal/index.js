...
function FinancialPaymentRefundAccountCreateModal({ store, receiptRepayId }) {
  ...
  useEffect(() => {
    accountBank && getProjList(accountBank)
  }, [accountBank])

  const onProjChange = (value) => {
    const findCurr = projList.find((item) => item.bankAccountId === value)
    const { accountOpeningDate, accountType, accountNumber } = findCurr ?? {}
    form.setFieldsValue({
      accountType,
  ...
  return (
    ...
    >
      <Form form={form} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} preserve={false}>
        ...
        <Item name="accountNumber" hidden>
          <Input />
        </Item>
      </Form>
    </Modal>
