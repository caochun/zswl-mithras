...
const urls = {
  xmlGetUrl: '/audit/common/record/getBpmnXml', // bpmn xml获取地址
  highLightGetUrl: '/audit/common/record/getProcessPictureData', // 高亮的节点线获取地址
}
const BpmnFlowChart = ({
  taskId, // 流程Id
  height, // 流程图高度
  ...
  const getData = async () => {
    ...
    try {
      const res = await Promise.all([
        ...
        http.get(ApiUrls.highLightGetUrl, { params: { taskId } }),
      ])
      if (res) {
        setLoading(false)
        setXmlData(res[0])
