...
const QuotationScheme = (
  { type, title, showValue, setShowValue, canEdit = true, isProjSponsor, rootStore, store },
  ref
) => {
  ...
  const CurrentDom = useMemo(() => {
    ...
    return Dom[type]
  }, [type, showValue, form, detailShowData])
  ...
  useEffect(() => {
    if (projectQSDetail) {
      ...
      const value = {
        ...
        approvedAmount: formatPercent(currentDetailData.approvedAmount),
