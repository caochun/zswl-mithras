...
const ReportList = () => {
  ...
  return <Table onFilter={(key,val) => saveServer('warehouse_search_1',val)} columnsFilter={'warehouse_search_1'} columns={columns} store={table} serial columnWidth={120}></Table>
}
...
const TaskList = ({ path }) => {
  ...
  return <Table onFilter={(key,val) => saveServer('warehouse_search_2',val)} columnsFilter={'warehouse_search_2'} columns={columns} store={table} serial scroll={{ x: 'auto' }}></Table>
}
