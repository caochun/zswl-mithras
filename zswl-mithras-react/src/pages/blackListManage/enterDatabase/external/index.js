...
function Index({ path }) {
  ...
  const columns = useMemo(() => {
    const nameColumns = [
      ...
      { title: '审批状态', search: { element: <Select options={application} allowClear /> } },
    ]
  ...
  return (
    <Page>
      <Table
        columnsFilter={'enterDatabase_external_1'}
                onFilter={(key,val) => saveServer('enterDatabase_external_1',val)}
