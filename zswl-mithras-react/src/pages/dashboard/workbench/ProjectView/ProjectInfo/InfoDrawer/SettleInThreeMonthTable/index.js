...
import { All_COLUMNS } from './Column.js'
...
const Index = ({ group, extraQueryParams }) => {
  ...
  const table = Table.useStore({
    request: async (params) => {
      const { list, sumData } = await Api.postProjectInfoSettleinthreemonthList({
        ...params,
        ...extraQueryParams,
      })
      setSumData(sumData)
  ...
  return (
    <Table
      extra={
        <ExportBtn
          tableStore={table}
          businessType={'DASHBOARD_PROJECT_SETTLE_IN_THREE_MONTH'}
