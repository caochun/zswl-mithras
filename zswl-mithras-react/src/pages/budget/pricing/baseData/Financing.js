import { Access, Button, ModalStore, Page, Table, TableStore } from '@zswl/components'
...
import financingCostsApi from '@/api/newFtp/financingCostsApi'
import { saveServer } from '@/utils'
import { BudgetFinancingCostEditModal as EditModal } from '@/components/Budget/PricingBaseDataEntries'
...
function Index({ path }) {
  ...
  const table = useMemo(() => {
    return new TableStore({
      request: async (params = {}) => {
        const { list = [], ...rest } = await financingCostsApi.postPricingList(params)
  ...
  const modal = useMemo(() => {
    return new ModalStore({
      ...
      onFinish: async (values) => {
        await financingCostsApi.postPricingModify(values)
  ...
  return (
    ...
    >
      <Table
      columnsFilter="pricing_baseData_Financing"
      onFilter={(key,val) => saveServer('pricing_baseData_Financing',val)}
        store={table}
        editable={false}
        scroll={{
          x: 'auto',
