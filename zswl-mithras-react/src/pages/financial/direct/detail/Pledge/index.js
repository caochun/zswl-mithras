...
const Index = ({
  ...
  //导出
  const exportPledge = async () => {
    const res = await Api.exportPledge({
      financingId,
    })
    if (res?.code === 200) {
      downFile(res)
      ...
      }),
      {
        ...
        render(val, t) {
          if (isFormApproval) {
            return (
              <JumpProjDetail
                title={t.projName?.value}
                projId={t.projReviewId?.value}
                isChange={t.projReviewId?.isChange}
              ></JumpProjDetail>
            )
          }
          return <JumpProjDetail title={t.projName} projId={t.projReviewId}></JumpProjDetail>
        },
      },
      {
        title: '合同编号',
