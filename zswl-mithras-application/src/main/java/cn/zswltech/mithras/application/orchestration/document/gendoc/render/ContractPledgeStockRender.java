 ...
 */
@Component
public class ContractPledgeStockRender extends AbstractContractRender<ContractPledge> {
    private static final String CORPORATION_TEMPLATE = "出质人：%s（盖章）\n    法定代表人（签字或签章）/授权代表：（签字）";
    ...
    public String render(OutputStream outputStream, ContractPledge contractPledge) throws Exception {
        ...
        InputStream inputStream = SpringUtil.getBean(FileTemplateService.class).getTemplate("合同-质押合同", "5.质押合同-股权.docx");
        ...
        // 查询租金
//        List<ContractRentActual> allRentList = SpringUtil.getBean(ContractRentActualService.class).listByContract(contractId);
        List<ContractRentEstimate> allRentList = SpringUtil.getBean(ContractRentEstimateService.class).listByContractId(contractId, null);
        long totalRent = allRentList.stream().filter(e -> Objects.nonNull(e.getRent())).mapToLong(ContractRentEstimate::getRent).sum();
        if (totalRent > 0) {
            BigDecimal b = BigDecimal.valueOf(totalRent).divide(BigDecimal.valueOf(10000), 0, RoundingMode.UP);
            renderMap.put(RenderParameterKeyHolder.TOTAL_RENT_CN, NumberChineseFormatter.format(b.longValue(), true, true));
            renderMap.put(RenderParameterKeyHolder.TOTAL_RENT, NumberUtil.decimalFormat(",##0", b));
        } else {
            renderMap.put(RenderParameterKeyHolder.TOTAL_RENT_CN, "              ");
            renderMap.put(RenderParameterKeyHolder.TOTAL_RENT, "              ");
        }
    ...
    protected boolean customShowFile(ContractPledge contractPledge) {
        FileTemplate templateRecord = SpringUtil.getBean(FileTemplateService.class).getTemplateRecord("合同-质押合同", "5.质押合同-股权.docx");
    ...
    private static class RenderParameterKeyHolder {
        ...
        // 租金总额（四舍五入到整数）
        public static final String TOTAL_RENT_CN = "totalRentCN";
        // 租金总额（四舍五入到整数）
        public static final String TOTAL_RENT = "totalRent";
        // 出质人表格
