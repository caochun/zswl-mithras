...
public class ClientMonitorService {
    ...
    public PageR<ClientMonitorListRsp> clientMonitorList(ClientMonitorListReq req) {
        ...
        Page<ClientMonitorListRsp> page = clientMapper.listMonitorClient(new Page<>(req.getPage(), req.getPageSize()), req.getClientName(), req.getOnlyWarn(), clients.getRecords().stream().map(Client::getId).collect(Collectors.toSet()));
        ...
        page.getRecords().forEach(item -> {
            ...
            if(ObjectUtil.isEmpty(item.getOpCount())){
                item.setOpCount(0);
            }
        });
