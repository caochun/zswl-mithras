package cn.zswltech.mithras.application.orchestration.document.gendoc.render.contract.baoli.yzgt;
 ...
 */
@Component
public class YzGtBaoLiTransferNotifyRender extends AbstractContractRender<ContractBaseInfo> {
    @Override
    public String render(OutputStream outputStream, ContractBaseInfo baseInfo) throws Exception {
        String filename = "1-3应收账款转让通知书及回执.docx";
        ...
        renderMap.put(CREDITOR_NAMES, StrUtil.join("、", tenantryList.stream().map(ContractTenantry::getLesseeName).collect(Collectors.toList())));
    ...
    public static final String CREDITOR_NAMES = "creditorNames";
