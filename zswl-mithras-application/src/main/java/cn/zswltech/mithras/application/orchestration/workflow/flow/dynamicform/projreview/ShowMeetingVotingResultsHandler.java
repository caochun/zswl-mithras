 ...
 */
@Component
public class ShowMeetingVotingResultsHandler implements DynamicFormHandler {
    ...
    public void collect(TaskDetailRSP rsp) {
        ...
        // 判断是否是存在评审主任
        Map<String, Object> variables = flowVariableApiService.getVariables(rsp.getProcessInstanceId(), Collections.singletonList(ProcessVarEnum.projReviewHasJuryDirector.name()));
        List<VoteResp> voteResps = new ArrayList<>();
        if(variables != null && variables.containsKey(ProcessVarEnum.projReviewHasJuryDirector.name())){
            boolean projReviewHasJuryDirector  = (boolean) variables.get(ProcessVarEnum.projReviewHasJuryDirector.name());
            if (projReviewHasJuryDirector){
                List<VoteResp> userTaskJuryMeetingReviewMember = processApiService.queryVoteResult(rsp.getProcessInstanceId(), "userTask_juryMeetingReview_member");
                List<VoteResp> userTaskJuryMeetingReviewDirector = processApiService.queryVoteResult(rsp.getProcessInstanceId(), "userTask_juryMeetingReview_director");
                voteResps.addAll(userTaskJuryMeetingReviewMember);
                voteResps.addAll(userTaskJuryMeetingReviewDirector);
            }else{
                voteResps = processApiService.queryVoteResult(rsp.getProcessInstanceId(), "userTask_juryMeetingReview");
            }
        }else {
            voteResps = processApiService.queryVoteResult(rsp.getProcessInstanceId(), "userTask_juryMeetingReview");
        }
        List<VoteFormRSP.VoteRSP> voteRSPList = voteResps
                .stream()
                // 一人一票 去重
    ...
    public FlowDynamicFormEnum getType() {
        return FlowDynamicFormEnum.projReview_showMeetingVotingResults;
    }
