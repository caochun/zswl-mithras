...
public class SetRiskFileHandler implements DynamicFormHandler {
    ...
    public void check(Map<String, Object> formMap, TaskResp taskResp, UserTaskExt userTaskExt) {
        ...
        if (BusinessModuleEnum.PROJ_REVIEW.equals(businessModuleEnum)) {
            if (!materialsListQueryService.exists(BusinessModuleEnum.PROJ_REVIEW.name(), ProjReviewMaterialsEnum.RISK_REVIEW_REPORT.name(), Long.valueOf(taskResp.getBusinessKey()))) {
            ...
            if (!materialsListQueryService.exists(BusinessModuleEnum.GROUP_CREDIT_REVIEW.name(), GroupCreditReviewMaterialsEnum.RISK_REVIEW_REPORT.name(), Long.valueOf(taskResp.getBusinessKey()))) {
