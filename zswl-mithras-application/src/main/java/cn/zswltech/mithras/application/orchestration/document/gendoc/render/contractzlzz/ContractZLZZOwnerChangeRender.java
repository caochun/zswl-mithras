 ...
 * @author dingqi
 * @date 2023/4/19
 * @description 租赁-直租-主合同-所有权转移证书
 */
@Component
public class ContractZLZZOwnerChangeRender extends AbstractContractZLZZRender<ContractBaseInfo> {
    @Override
    public String render(OutputStream outputStream, ContractBaseInfo contractBaseInfo) throws Exception {
        ...
        // 渲染文档
        InputStream inputStream = getBean(FileTemplateService.class).getTemplate("合同-直租合同", "1-9.所有权转移证书.docx");
        XWPFTemplate template = XWPFTemplate.compile(inputStream).render(renderMap);
        template.writeAndClose(outputStream);
        return ContractSubTypeEnum.ZL_ZZ_OWNER_CHANGE.display() + GlobalConstants.OFFICE_WORD_SUFFIX;
    }
