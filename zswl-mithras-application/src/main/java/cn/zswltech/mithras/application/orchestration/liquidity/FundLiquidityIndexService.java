...
public class FundLiquidityIndexService implements FundLiquidityIndexApplicationService {
    ...
    public List<LiquidityMismatchDetailRSP> manageMismatch(LiquidityMismatchDetailREQ req) {
        ...
        // 实际租金表，还本付息维度
        Map<Long, List<LiquidityContractRentSnapshot>> contractActualMap = buildContractActualByReceiptId();
        ...
        Map<Long, LocalDate> receiptMap = LiquidityIndicatorMismatchHolder.FUND_RECEIPT_REPAY_BASE_INFO.values().stream().filter(f -> financingIdList.contains(f.getFinancingId()))
                .collect(Collectors.toMap(LiquidityFundReceiptRepaySnapshot::getId, receipt -> {
        ...
        Map<Long, List<LiquidityFundReceiptFlowPlanSnapshot>> flowPlanMap = LiquidityIndicatorMismatchHolder.FUND_RECEIPT_FLOW_PLAN.values().stream().map(Map::values).flatMap(Collection::stream).flatMap(Collection::stream).filter(f -> {
            return receiptMap.containsKey(f.getReceiptRepayId()) &&
                    Objects.equals(f.getCashFlowItem(), FinanceCashFlowItemEnum.REPAY.name());
        }).collect(Collectors.groupingBy(LiquidityFundReceiptFlowPlanSnapshot::getReceiptRepayId));

        for (Map.Entry<Long, List<LiquidityFundReceiptFlowPlanSnapshot>> entry : flowPlanMap.entrySet()) {
            List<LiquidityContractRentSnapshot> contractRentActualList = contractActualMap.get(entry.getKey());
            if (CollectionUtil.isNotEmpty(contractRentActualList)) {
                List<LiquidityFundReceiptFlowPlanSnapshot> flowPlanListSort = entry.getValue().stream().sorted(Comparator.comparing(LiquidityFundReceiptFlowPlanSnapshot::getCashFlowPhase)).collect(Collectors.toList());
                ...
                // 合同下的现金流维度
                for (LiquidityFundReceiptFlowPlanSnapshot flowPlan : flowPlanListSort) {
                    ...
                    if (!flowPlan.getCashFlowDate().isBefore(req.getQueryDateStart()) && !flowPlan.getCashFlowDate().isAfter(req.getQueryDateEnd())) {
                        ...
                        List<LiquidityContractRentSnapshot> matchContractRentActualList = contractRentActualList.stream().filter(f -> {
                            // 匹配到租金表中的应还日期
    ...
    public LiquidityMismatchDetailRSP buildResult(LiquidityFundReceiptFlowPlanSnapshot flowPlan, List<LiquidityContractRentSnapshot> matchContractRentActualList) {
        ...
        if (CollectionUtil.isNotEmpty(matchContractRentActualList)) {
            cashInFlowList = matchContractRentActualList.stream().map(item -> {
                ...
                LiquidityContractBaseSnapshot contractBaseInfo = LiquidityIndicatorMismatchHolder.CONTRACT_BASE_INFO.get(item.getContractId());
                cashInFlow.setPledgeContractCode(Optional.ofNullable(contractBaseInfo).map(LiquidityContractBaseSnapshot::getContractCode).orElse(null));
                return cashInFlow;
            }).collect(Collectors.toList());
        }

        LiquidityFundReceiptRepaySnapshot receiptRepayBaseInfo = LiquidityIndicatorMismatchHolder.FUND_RECEIPT_REPAY_BASE_INFO.get(flowPlan.getReceiptRepayId());
        if (receiptRepayBaseInfo.getFinancingType() == null) {
            // 间融
            LiquidityFinancingSnapshot financingBaseInfo = LiquidityIndicatorMismatchHolder.FUND_FINANCING_BASE_INFO.get(receiptRepayBaseInfo.getFinancingId());
            List<LiquidityFinancingOrganizationSnapshot> organizationList = LiquidityIndicatorMismatchHolder.FUND_ORGANIZATION_INFO.get(receiptRepayBaseInfo.getFinancingId());
            if (CollectionUtil.isNotEmpty(organizationList)) {
                rsp.setOrganizationId(organizationList.stream().map(LiquidityFinancingOrganizationSnapshot::getId).collect(Collectors.toList()));
                rsp.setOrganizationName(organizationList.stream().map(LiquidityFinancingOrganizationSnapshot::getOrganizationName).collect(Collectors.toList()));
            }
            rsp.setFinancingCode(financingBaseInfo.getFinancingCode());
        } else {
            // 直融
            LiquidityDirectFinancingSnapshot directFinancingBaseInfo = LiquidityIndicatorMismatchHolder.FUND_DIRECT_FINANCING_BASE_INFO.get(receiptRepayBaseInfo.getFinancingId());
    ...
    public Map<Long, List<LiquidityContractRentSnapshot>> buildContractActualByReceiptId() {
        // 质押监管明细
//        Map<Long, LiquidityFinancingPledgeSnapshot> inDirectPledgeMap = LiquidityIndicatorMismatchHolder.FUND_FINANCING_PLEDGE_INFO.values().stream().flatMap(Collection::stream)
//                .collect(Collectors.toMap(LiquidityFinancingPledgeSnapshot::getContractId, Function.identity()));
        Map<Long, List<LiquidityFinancingPledgeSnapshot>> inDirectPledgeMapTemp = LiquidityIndicatorMismatchHolder.FUND_FINANCING_PLEDGE_INFO.values().stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(LiquidityFinancingPledgeSnapshot::getContractId));
        Map<Long, LiquidityFinancingPledgeSnapshot> inDirectPledgeMap = new HashMap<>();
        if (CollectionUtil.isNotEmpty(inDirectPledgeMapTemp)) {
            for (Map.Entry<Long, List<LiquidityFinancingPledgeSnapshot>> entry : inDirectPledgeMapTemp.entrySet()) {
                List<LiquidityFinancingPledgeSnapshot> list = entry.getValue();
                if (list.size() == 1) {
                    inDirectPledgeMap.put(entry.getKey(), list.get(0));
                } else {
                    // 优先取监管，再取质押
                    List<LiquidityFinancingPledgeSnapshot> superviseList = list.stream().filter(e -> Objects.nonNull(e.getSupervise()) && e.getSupervise()).collect(Collectors.toList());
                    List<LiquidityFinancingPledgeSnapshot> pledgeList = list.stream().filter(e -> Objects.nonNull(e.getPledge()) && e.getPledge()).collect(Collectors.toList());
...
//        Map<Long, LiquidityFinancingPledgeSnapshot> directPledgeMap = LiquidityIndicatorMismatchHolder.FUND_DIRECT_FINANCING_PLEDGE_INFO.values().stream().flatMap(Collection::stream)
//                .collect(Collectors.toMap(LiquidityFinancingPledgeSnapshot::getContractId, Function.identity()));
        Map<Long, List<LiquidityFinancingPledgeSnapshot>> directPledgeMapTemp = LiquidityIndicatorMismatchHolder.FUND_DIRECT_FINANCING_PLEDGE_INFO.values().stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(LiquidityFinancingPledgeSnapshot::getContractId));
        Map<Long, LiquidityFinancingPledgeSnapshot> directPledgeMap = new HashMap<>();
        if (CollectionUtil.isNotEmpty(directPledgeMapTemp)) {
            for (Map.Entry<Long, List<LiquidityFinancingPledgeSnapshot>> entry : directPledgeMapTemp.entrySet()) {
                List<LiquidityFinancingPledgeSnapshot> list = entry.getValue();
                if (list.size() == 1) {
        ...
        Map<Long, Long> idByReceiptIdInDirectMap = LiquidityIndicatorMismatchHolder.FUND_RECEIPT_REPAY_BASE_INFO.values().stream().filter(f -> Objects.isNull(f.getFinancingType()))
                .collect(Collectors.toMap(LiquidityFundReceiptRepaySnapshot::getFinancingId, LiquidityFundReceiptRepaySnapshot::getId));
        ...
        Map<Long, Long> receiptIdMap = contractIdList.stream().collect(Collectors.toMap(Function.identity(), contractId -> {
            LiquidityFinancingPledgeSnapshot pledgeInfoInDirect = inDirectPledgeMap.get(contractId);
            LiquidityFinancingPledgeSnapshot pledgeInfoDirect = directPledgeMap.get(contractId);
            Long inDirectReceiptId = Optional.ofNullable(pledgeInfoInDirect).map(LiquidityFinancingPledgeSnapshot::getFinancingId).map(idByReceiptIdInDirectMap::get).orElse(null);
            Long directReceiptId = Optional.ofNullable(pledgeInfoDirect).map(LiquidityFinancingPledgeSnapshot::getFinancingId).map(idByReceiptIdDirectMap::get).orElse(null);
    ...
    public List<LiquidityBoardRentIncomeRSP> manageRentIncome(LiquidityBoardRentIncomeREQ req) {
        ...
        Map<Long, String> contractCodeMap = contractBaseInfoList.stream()
                .collect(Collectors.toMap(ContractBaseInfo::getId, ContractBaseInfo::getContractCode, (m1, m2) -> m1));
        ...
        List<LiquidityCollectionPlanSnapshot> actualCollectionBaseInfoList = new ArrayList<>();
        ...
        List<LiquidityCollectionPlanSnapshot> collectionPlanList = actualCollectionBaseInfoList.stream().filter(f -> {
        ...
        collectionPlanList = collectionPlanList.stream().map(m -> {
            ...
            if (Objects.equals(m.getPhase(), maxPhase)) {
                ...
                return new LiquidityCollectionPlanSnapshot(m.getId(), m.getContractId(), m.getCode(), m.getPhase(), m.getPlanCollectionDate(), result >= 0 ? result : 0);
            }
            return m;
        }).collect(Collectors.toList());
        List<ContractTenantry> tenantryList = contractTenantryService.list(Wrappers.<ContractTenantry>lambdaQuery()
                .in(ContractTenantry::getContractId, collectionPlanList.stream().map(LiquidityCollectionPlanSnapshot::getContractId).collect(Collectors.toList())));
        ...
        for (LiquidityCollectionPlanSnapshot collectionBaseInfo : collectionPlanList) {
            LiquidityBoardRentIncomeRSP rsp = new LiquidityBoardRentIncomeRSP();
            rsp.setContractId(collectionBaseInfo.getContractId());
            rsp.setContractCode(contractCodeMap.get(collectionBaseInfo.getContractId()));
            ...
            LiquidityBankAccountSnapshot baseDataBankAccount = BASE_DATA_BANK_ACCOUNT.get(accountId);
