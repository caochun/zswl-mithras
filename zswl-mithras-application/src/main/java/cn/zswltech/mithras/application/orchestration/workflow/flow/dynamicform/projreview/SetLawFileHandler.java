 ...
 */
@Component
public class SetLawFileHandler implements DynamicFormHandler {
    ...
    public void check(Map<String, Object> formMap, TaskResp taskResp, UserTaskExt userTaskExt) {
        ...
        if (BusinessModuleEnum.PROJ_REVIEW.equals(businessModuleEnum)) {
            if (!materialsListQueryService.exists(BusinessModuleEnum.PROJ_REVIEW.name(), ProjReviewMaterialsEnum.LEGAL_COMPLIANCE_REPORT.name(), Long.valueOf(taskResp.getBusinessKey()))) {
                throw new MithrasException("请上传" + ProjReviewMaterialsEnum.LEGAL_COMPLIANCE_REPORT.getDisplay());
            }
        } else if (BusinessModuleEnum.GROUP_CREDIT_REVIEW.equals(businessModuleEnum)) {
            if (!materialsListQueryService.exists(BusinessModuleEnum.GROUP_CREDIT_REVIEW.name(), GroupCreditReviewMaterialsEnum.LEGAL_COMPLIANCE_REPORT.name(), Long.valueOf(taskResp.getBusinessKey()))) {
                throw new MithrasException("请上传" + GroupCreditReviewMaterialsEnum.LEGAL_COMPLIANCE_REPORT.getDisplay());
            }
        }
    }
    ...
    public FlowDynamicFormEnum getType() {
        return FlowDynamicFormEnum.projReview_setLawFile;
    }
