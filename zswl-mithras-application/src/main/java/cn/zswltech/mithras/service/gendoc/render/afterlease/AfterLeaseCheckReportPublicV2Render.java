 ...
 */
@Component
public class AfterLeaseCheckReportPublicV2Render extends AbstractAfterLeaseCheckReportPublicRender {
    @Override
    protected AfterLeaseCheckReportTemplateVersionEnum getTemplateVersion() {
        return AfterLeaseCheckReportTemplateVersionEnum.PUBLIC_V2;
    }
    ...
    public String render(OutputStream outputStream, NewAfterLeaseCheckPlanClient checkPlanClient) throws Exception {
        ...
        Map<String, NewAfterLeaseCheckReportDetail.FieldData> fieldDataMap = checkReportBO.getFieldDataList().stream().filter(e -> Objects.equals(e.getModuleIndex(), 0)).collect(Collectors.toMap(NewAfterLeaseCheckReportDetail.FieldData::getFieldName, e -> e));
        // 补充V2版本的数据
