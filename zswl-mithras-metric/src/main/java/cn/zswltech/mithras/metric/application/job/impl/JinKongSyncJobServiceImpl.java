package cn.zswltech.mithras.metric.application.job.impl;
...
import cn.zswltech.mithras.third.yunhu.client.YunHuReportIndicatorDataHandler;
import cn.zswltech.mithras.third.yunhu.client.req.ReportIndicatorDataReq;
import cn.zswltech.mithras.third.yunhu.client.res.ReportIndicatorDataRes;
...
public class JinKongSyncJobServiceImpl implements JinKongSyncJobService {
    @Resource
    private JinKongMonthlyReportPort jinKongMonthlyReportPort;
    @Resource
...
//            String s = "{\"year\":2025,\"month\":4}";
            ...
            jinKongMonthlyReportPort.syncAccountBalanceData(year, month);
            try {
                //同步保存科目余额辅助表
                jinKongMonthlyReportPort.syncBcmFflexfiledAssistMfByDept(year, month);
            } catch (Exception e) {
    ...
    public void jinKongSyncAssetJob(String jobParam) {
        try {
            ...
            jinKongMonthlyReportPort.jinKongSyncAsset(targetDate);
        } catch (Exception e) {
    ...
    public void jinKongSyncProfitJob(String jobParam) {
        try {
            ...
            jinKongMonthlyReportPort.jinKongSyncProfit(targetDate);
        } catch (Exception e) {
    ...
    public void jinKongSyncCashflowJob(String jobParam) {
        try {
            ...
            jinKongMonthlyReportPort.jinKongSyncCashFlow(targetDate);
        } catch (Exception e) {
