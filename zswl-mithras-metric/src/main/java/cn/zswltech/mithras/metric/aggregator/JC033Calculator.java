 ...
 */
@Component
public class JC033Calculator implements MetricCalculator {
    ...
    public Map<String, Long> varMap(LocalDate date) {
        ...
        RiskMetricDict dict = metricDictService.getOne(Wrappers.<RiskMetricDict>lambdaQuery()
                ...
                .eq(RiskMetricDict::getDictKey, PUBLIC_UTILITIES.name())
        );
        result.put("a", Long.parseLong(dict.getDickValue()) * 100_000_000 * 10000L);
        //
        ...
        wrapper.eq("proj_type", PUBLIC_UTILITIES.name());
