...
@Data
public class RiskEventUpdateReqSingleEvent {
    private String eventCode;//	string	必须	事件编号
    ...
    private BigDecimal currentRecoverableAmount; //number 必须 当期收回金额 小数点前最大15位，保留两位小数
    private BigDecimal writeOffAmount; //number 必须 累计核销金额 小数点前最大15位，保留两位小数
    private BigDecimal needRecoverableAmount; //number 必须 待收回金额 小数点前最大15位，保留两位小数
    private String currentDealProgress;// string 必须 当期处置举措及进展
    private Boolean adjustRiskLevel;//boolean 必须 是否调整事件风险等级
