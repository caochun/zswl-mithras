...
当前模块 POM 和 Java import 都保持低耦合，直接依赖只有 api、foundation 和框架 provided 依赖。代码层 import 粗分约为 `blackgray` 336 处、`foundation` 47 处、`api` 36 处、`dto` 12 处，没有直接引用其他业务域包。客户详情、审批流程启动已经分别通过 `BlackGrayCustomerPort`、`BlackGrayApprovalProcessPort` 由 application 适配，方向是对的。
