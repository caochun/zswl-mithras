...
@Slf4j
@Order(-1)
public class CrMortgageHandler extends CrAbstractHandler<CrMortgageDraft, CrMortgage> {
    ...
    public ReportModuleEnum reportModule() {
        return ReportModuleEnum.MORTGAGE;
    }
    ...
    protected void moduleHandle(LocalDateTime dealTime, LocalDateTime lastDealTime) {
        ...
        Map<String, CrMortgageDraft> existReportDataMap = existDataList.stream().collect(Collectors.toMap(CrMortgageDraft::getBusinessKey, cr -> cr));
        ...
        log.info("直租征信报送-抵押表，此次处理数量:{}, 处理businessKey列表:{}, 删除businessKey列表:{}", reportDataList.size(), JSON.toJSONString((businessKeySet)), JSON.toJSONString(needDeleteKeyList));
    ...
    public List<CrMortgageDraft> collectContractChangeData(List<ContractBaseInfoLib> contractBaseInfoLibList) {
        ...
        for (ContractBaseInfoLib contractBaseInfoLib : contractBaseInfoLibList) {
            ...
            for (PaymentBaseInfo paymentBaseInfo : writtenOffPaymentList) {
                resultList.addAll(
                        contractMortgageItemLibList.stream().map(e -> buildContractMortagage(paymentBaseInfo, contractBaseInfoLib, contractMortgageLibMap, e))
    ...
    private CrMortgageDraft buildContractMortagage(PaymentBaseInfo paymentBaseInfo,
                                                   ContractBaseInfoLib contractBaseInfoLib,
                                                   Map<Long, ContractMortgageLib> contractMortgageLibMap,
                                                   ContractMortgageItemLib c) {
        ...
        if (Objects.nonNull(contractMortgageLib)) {
            ...
            crMortgage.setReportState(ReportState.TO_BE_REPORT.name())
                    ...
                    .setMaxFlag(contractMortgageLib.getHighest())
                    .setSequence(String.valueOf(c.getSequence()))
                    .setType(MortgageDataTypeEnum.convert(c.getCategory()))
                    .setModelType(MortgageModelTypeEnum.convert(c.getUniqueIdentifyCodeType()))
                    .setModel(ReportBizUtil.removeSpecialChar(c.getUniqueIdentifyCode()))
                    .setAssessedValue(c.getAssessedNetValue())
                    .setAppraisalCompanyType(MortgageAppraisalCompanyTypeEnum.convert(contractMortgageLib.getAssess()))
                    .setAssessedDate(contractMortgageLib.getAssessDate())
                    .setMortgageDescribe(ReportBizUtil.removeSpecialChar(contractMortgageLib.getMortgageDescribe()));
            crMortgage.setBusinessKey(crMortgage.genBusinessKey(c.getOriginId()));
            crMortgage.setMortgageContractCode(ReportBizUtil.contractCodeAddPaymentSeq(ReportBizUtil.removeSpecialChar(contractMortgageLib.getMortgageContractCode()), crMortgage.getPaymentApplyCode()));
    ...
    private void fillClientData(List<CrMortgageDraft> crMortgageList) {
        if (CollectionUtils.isEmpty(crMortgageList)) {
            return;
        }
        ...
        crMortgageList.forEach(cr -> {
            ...
            cr.setMortgageIdType(ClientType.CORPORATION.name().equals(client.getClientType()) ? ReportClientTypeEnum.CORPORATION.getCode() : client.getCertType());
