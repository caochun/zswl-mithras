...
public class FCM_093Calculator implements FinancialCloudMetricCalculator {
    ...
    public BigDecimal calculate(LocalDate dateTime) {
        ...
        // 本年新增融资
        List<PaymentActualDetail> detailList = paymentActualDetailService.list(Wrappers.<PaymentActualDetail>lambdaQuery()
        ...
        List<PaymentBaseInfo> paymentBaseInfos = paymentBaseInfoService.listByIds(paymentIds);
        // 寻找借据
        List<ContractReceipt> contractReceiptList = contractReceiptService.list(Wrappers.<ContractReceipt>lambdaQuery()
                // 这里暂时只查生效的
        ...
        for (PaymentActualDetail actualDetail : actualDetails) {
            ...
            FtpAssessmentInfo latestEffect = ftpAssessmentInfoService.findLatestEffect(dateTime, receipt.getId());
