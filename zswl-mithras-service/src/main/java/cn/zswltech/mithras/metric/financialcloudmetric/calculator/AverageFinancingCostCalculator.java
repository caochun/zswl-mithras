...
import cn.zswltech.mithras.fund.infrastructure.persistence.mapper.receiptrepay.FundReceiptFlowPlanMapper;
...
public abstract class AverageFinancingCostCalculator implements FinancialCloudMetricCalculator {
    ...
    @Resource
    protected FundReceiptRepayBaseInfoService repayBaseInfoService;
    ...
    protected BigDecimal cal(LocalDate dateTime) {
        ...
        if (COST.isEmpty()) {
            synchronized (lock) {
                if (COST.isEmpty()) {
                    Map<Long, Integer> collect = financingPlanService.list(Wrappers.<FundFinancingPlan>lambdaQuery())
        ...
        // 这里不能取融资金额，需要使用剩余本金
        Map<Long, Long> repayPrincipalMap = repayBaseInfoService.queryRemainingAmount(financingIds, FinancingTypeEnum.INDIRECT);
