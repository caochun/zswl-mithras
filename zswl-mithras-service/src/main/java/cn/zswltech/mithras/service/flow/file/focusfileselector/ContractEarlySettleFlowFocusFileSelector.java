 ...
 * @author dingqi
 * @date 2024/2/17
 * @description 合同提前结清流程-关注文件筛选
 */
@Component
public class ContractEarlySettleFlowFocusFileSelector extends AbstractContractFlowFocusFileSelector {
    @Override
    protected List<String> bizImportantFileTypes() {
        return Arrays.asList(
                ...
                ContractExtraFileTypeEnum.CONTRACT_SETTLE.name()
        );
    }

    @Override
    public ProcessModelTypeEnum processType() {
        return ProcessModelTypeEnum.ContractEarlySettleFlow;
    }
}
