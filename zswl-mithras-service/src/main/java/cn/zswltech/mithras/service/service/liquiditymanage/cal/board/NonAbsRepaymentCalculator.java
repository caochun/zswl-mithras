 ...
 * 非ABS还款(元)
 *
 * 账户明细表中当日全部账户的还本付息(非abs）之和
 * @author chenyifei
 * @since 2024/12/20
 */
@Component
public class NonAbsRepaymentCalculator extends AbstractLiquidityCalculator<LiquidityBoardCalculatorBo> {
    ...
    public void calculate(Object obj, LiquidityBoardCalculatorBo bo) {
        ...
        if(CollectionUtil.isNotEmpty(accountBalanceMap)){
            result = accountBalanceMap.values().stream().map(m -> {
                return LongUtil.null2zeroBigDecimal(m.getRepayNoAbsAmount());
