 ...
 * 债务偿还（元）
 *
 * 账户明细表中当日全部账户的（还本付息+还本付息（调整值））之和
 * @author chenyifei
 * @since 2024/12/20
 */
@Component
public class DebtRepaymentCalculator extends AbstractLiquidityCalculator<LiquidityBoardCalculatorBo> {
    ...
    public void calculate(Object obj, LiquidityBoardCalculatorBo bo) {
        ...
        if(CollectionUtil.isNotEmpty(accountBalanceMap)){
            result = accountBalanceMap.values().stream().map(m -> {
                return LongUtil.null2zeroBigDecimal(m.getRepayAmount()).add(LongUtil.null2zeroBigDecimal(m.getRepayEditAmount()));
