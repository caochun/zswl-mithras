...
import cn.zswltech.mithras.service.mapper.model.monthly.MonthlyFinanceStampDuty;
...
import cn.zswltech.mithras.service.mapper.monthly.MonthlyFinanceStampDutyMapper;
...
public class MonthlyFinanceStampDutyService extends ServiceImpl<MonthlyFinanceStampDutyMapper, MonthlyFinanceStampDuty> {
    ...
    public PageR<MonthlyStampDutyFinRSP> finPage(MonthlyStampDutyFinREQ req) {
        ...
        Page<MonthlyFinanceStampDuty> pageRecord = monthlyFinanceStampDutyService.page(page, Wrappers.<MonthlyFinanceStampDuty>lambdaQuery()
                .like(CharSequenceUtil.isNotBlank(req.getFinancingCode()), MonthlyFinanceStampDuty::getFinancingCode, req.getFinancingCode())
                .like(CharSequenceUtil.isNotBlank(req.getOrganizationName()), MonthlyFinanceStampDuty::getOrganizationName, req.getOrganizationName())
        ...
        if (CollUtil.isNotEmpty(monthlyStampDutyList)) {
            monthlyStampDutyList.forEach(monthlyStampDuty -> {
                MonthlyStampDutyFinRSP copied = BeanUtil.copyProperties(monthlyStampDuty, MonthlyStampDutyFinRSP.class);
    ...
    public void freshFinanceStampDuty(MonthlyFreshREQ req) {
        MonthlyManagementBaseInfo baseInfo = baseInfoService.getMonthlyManagementBaseInfo(req.getYearAndMonth());
        LocalDate currentDate = baseInfoService.handleDate(req.getYearAndMonth());
        ...
        List<MonthlyStampDuty> monthlyStampDutyList = stampDutyService.page(page, Wrappers.<MonthlyStampDuty>lambdaQuery()
        ...
        if (CollectionUtils.isNotEmpty(monthlyStampDutyList)) {
            monthlyStampDutyList.stream().forEach(dto -> {
                ...
                MonthlyFinanceStampDuty financeStampDuty = BeanUtil.copyProperties(dto, MonthlyFinanceStampDuty.class, GlobalConstants.COPY_IGNORE_COMMON_FIELD);
