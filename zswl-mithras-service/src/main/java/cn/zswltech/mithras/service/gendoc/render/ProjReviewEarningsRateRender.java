...
import cn.zswltech.mithras.service.enums.newftp.AssetIndustryClassify;
 ...
 * @deprecated 该类废弃，由cn.zswltech.mithras.service.flow.dynamicform.projreview.SetPricingChooseApproveAuthHandler功能替代
 */
@Deprecated
@Slf4j
@Component
public class ProjReviewEarningsRateRender extends AbstractBasicRender<ProjReviewRenderBO> {
    @Resource
    ...
    @Resource
    private ProjReviewCashFlowPlanMapper projReviewCashFlowPlanMapper;
    @Resource
    ...
    public String render(OutputStream outputStream, ProjReviewRenderBO projReviewRenderBO) throws Exception {
        ...
        Map<Long, OrgDO> orgMap = businessDataRepository.getOrgMap(Collections.singletonList(projReviewBaseInfo.getBizDeptId()));
        OrgDO orgDO = orgMap.get(projReviewBaseInfo.getBizDeptId());
        ProjReviewPriceDetailRSP priceDetail = projReviewPriceService.detail(projReviewBaseInfo.getId());
        ...
        //查询客户信息
        CorpCommerceInfo corpCommerceInfo = corpCommerceInfoService.detail(projReviewBaseInfo.getClientId(), null);
        ...
        renderMap.put(RenderParameterKeyHolder.PROJ_NAME, projReviewBaseInfo.getProjName());
        renderMap.put(RenderParameterKeyHolder.BIZ_DEPT, Optional.ofNullable(orgDO).map(OrgDO::getName).orElse(""));
        renderMap.put(RenderParameterKeyHolder.PROJECT_CLASSIF, Optional.ofNullable(ProjectClassify.find(projReviewBaseInfo.getProjectClassify())).map(ProjectClassify::display).orElse("   "));
...
//            renderMap.put(RenderParameterKeyHolder.REGIONAL_PROJECT_CLASSIFY, ProjRegionalDivisionEnum.getProjReviewEarningsString(ProjRegionalDivisionEnum.of(projReviewBaseInfo.getRegionalDivision())));
//        } else {
//            //当风控行业分类≠公用事业类、民生消费类、旅游行业时按地区分类进行区分，①若地区分类为「浙江地区」则展示为“浙江地区”；②若地区分类为「鼓励支持类」则展示为“鼓励支持类地区”；③若地区分类为「适度支持类」或「谨慎支持类」则展示为“其他地区”。
//            renderMap.put(RenderParameterKeyHolder.REGIONAL_PROJECT_CLASSIFY, RegionalClassify.getProjRegionalClassify(projReviewBaseInfo.getRegionalProjectClassify()).display());
//        }
        RegionalClassify regionalClassify;
        if (StrUtil.equalsAny(projReviewBaseInfo.getFtpIndustryCategory(), FtpIndustryCategoryEnum.FTP_STATE_OWNED_INDUSTRY.name(), FtpIndustryCategoryEnum.FTP_PUBLIC_UTILITIES.name(), FtpIndustryCategoryEnum.FTP_CIVIL_CONSUMPTION.name())) {
            regionalClassify = ProjRegionalDivisionEnum.getProjReviewEarningsRegionalClassify(ProjRegionalDivisionEnum.of(projReviewBaseInfo.getRegionalDivision()));
        } else {
            regionalClassify = RegionalClassify.getProjRegionalClassify(projReviewBaseInfo.getRegionalProjectClassify());
        }
        if (Objects.isNull(regionalClassify)) {
            log.error("无法确定FTP地区分类，使用其他地区进行兜底[{}]", projReviewBaseInfo.getProjCode());
...
//        }
        ...
        //获取项目承租人
        List<Long> projReviewClientIds = getProjReviewClientIds(projReviewBaseInfo);
        ...
        if (Objects.isNull(customerEntityClassify)) {
            log.error("无法确定FTP客户主体分类，使用其他进行兜底[{}]", projReviewBaseInfo.getProjCode());
        ...
        } else if (ProjectClassify.CONSTRUCTION_MACHINERY.name().equals(projReviewBaseInfo.getProjectClassify())) {
            ftpQuarterlyBasePricingBO.setAssetIndustryClassify(AssetIndustryClassify.ENCOURAGE_INTERVENTION.name());
        }
        ...
        //折现率
        BigDecimal discountRate = getCashFlowBOS(projReviewRenderBO);
        renderMap.put(RenderParameterKeyHolder.DISCOUNT_RATE, discountRate);
        //构建参数
        BigDecimal cashFtp = null;
        //todo 代码报错，先捕捉一下
        List<PaymentBaseInfo> paymentBaseInfoList = paymentBaseInfoService.listEffectPaymentByContractId(projReviewBaseInfo.getClientId());
        ...
        try {
            cashFtp = LongUtil.tenThousand2Dollar(String.valueOf(ftpService.getCashFtp(getCashFtpInfluenceBO(projReviewBaseInfo, priceDetail, corpCommerceInfo, ftpQuarterlyBasePricingBO.getRegionalClassify()), paymentIdList))).setScale(2, RoundingMode.HALF_UP);
        ...
        if (StrUtil.isNotBlank(projReviewBaseInfo.getLeaseTypes())) {
            ...
            if (ProjectBizType.ZL.name().equals(projReviewBaseInfo.getBizType()) || ProjectBizType.ZZ.name().equals(projReviewBaseInfo.getBizType())) {
...
//        }
        ...
        XWPFTemplate template = XWPFTemplate.compile(ProjReviewEarningsRateRender.class.getResourceAsStream(templatePath)).render(renderMap);
        template.writeAndClose(outputStream);

        return projReviewBaseInfo.getProjName() + "-" + ProjReviewMaterialsEnum.YIELD_REVIEW_REPORT.getDisplay() + GlobalConstants.OFFICE_WORD_SUFFIX;
    }

    @NotNull
    private BigDecimal getCashFlowBOS(ProjReviewRenderBO projReviewRenderBO) {
        //获取现金流计划
        LambdaQueryWrapper<ProjReviewCashFlowPlan> PlanQueryWrapper = Wrappers.<ProjReviewCashFlowPlan>lambdaQuery()
                .eq(Objects.nonNull(projReviewRenderBO.getProjReviewBaseInfo().getId()), ProjReviewCashFlowPlan::getProjectId, projReviewRenderBO.getProjReviewBaseInfo().getId());
        List<ProjReviewCashFlowPlan> projReviewBaseInfoList = projReviewCashFlowPlanMapper.selectList(PlanQueryWrapper);
        //获取现金流计划
        Wrapper<ProjReviewLeasePrice> priceQueryWrapper = Wrappers.<ProjReviewLeasePrice>lambdaQuery()
                .eq(Objects.nonNull(projReviewRenderBO.getProjReviewBaseInfo().getId()), ProjReviewLeasePrice::getProjectId, projReviewRenderBO.getProjReviewBaseInfo().getId());
        ProjReviewLeasePrice projReviewLeasePrice = projReviewLeasePriceMapper.selectOne(priceQueryWrapper);

        List<CashFlowBO> cashFlowBOList = projReviewBaseInfoList.stream().map(item -> {
            ...
            if (Objects.nonNull(projReviewLeasePrice.getRepayTimesTotal())
                    ...
                    && projReviewLeasePrice.getRepayTimesTotal().equals(item.getCashFlowPhase())) {
                cashFlowBO.setCashFlowAmount(item.getRent() - projReviewLeasePrice.getEarnestMoney());
            } else {
        ...
        Long cashFlowAmount = -1 * projReviewLeasePrice.getApplyCreditAmount()
                + Optional.ofNullable(projReviewLeasePrice.getDownPayment()).orElse(0L)
                + Optional.ofNullable(projReviewLeasePrice.getEarnestMoney()).orElse(0L)
                + Optional.ofNullable(projReviewLeasePrice.getCommission()).orElse(0L)
                + Optional.ofNullable(projReviewLeasePrice.getFirstInstallmentInterest()).orElse(0L);
        //方法需要0期限的数据
        CashFlowBO cashFlowBO = new CashFlowBO()
                .setCashFlowAmount(cashFlowAmount)
                .setCashFlowDate(projReviewLeasePrice.getPlannedStartingDate())
                .setCashFlowPhase(0);
    ...
    private CashFtpInfluenceBO getCashFtpInfluenceBO(ProjReviewBaseInfo projReviewBaseInfo, ProjReviewPriceDetailRSP priceDetail, CorpCommerceInfo corpCommerceInfo, String regionalClassify) {
        CashFtpInfluenceBO cashFtpInfluenceBO = new CashFtpInfluenceBO();
        cashFtpInfluenceBO.setJudgePledge(false);
        cashFtpInfluenceBO.setBizType(projReviewBaseInfo.getBizType());
        ...
        if (Objects.equals(projReviewBaseInfo.getProjectClassify(), ProjectClassify.CONSTRUCTION_MACHINERY.name())) {
//            cashFtpInfluenceBO.setRiskControlIndustryClassify(RiskControlIndustryClassify.ENGINEERING_MACHINERY.name());
        ...
        } else if (Objects.equals(projReviewBaseInfo.getProjectClassify(), ProjectClassify.INTRA_GROUP_COLLABORATION.name())) {
            cashFtpInfluenceBO.setRiskControlIndustryClassify(RiskControlIndustryClassify.INTRA_GROUP_COLLABORATION.name());
            cashFtpInfluenceBO.setAssetIndustryClassify(null);
        }
        cashFtpInfluenceBO.setTenantId(projReviewBaseInfo.getClientId());
        if (StrUtil.isNotBlank(projReviewBaseInfo.getGuaranteeInfo())) {
            ...
            if (CollectionUtil.isNotEmpty(guaranteeList)) {
                cashFtpInfluenceBO.setGuarantorIdList(guaranteeList.stream().map(ClientInfo::getClientId).collect(Collectors.toList()));
            }
        }
        return cashFtpInfluenceBO;
    }

    private List<Long> getProjReviewClientIds(ProjReviewBaseInfo projReviewBaseInfo) {
        ProjectBizType projectBizType = ProjectBizType.of(projReviewBaseInfo.getBizType());
        List<Long> clientIds = new ArrayList<>();
        switch (projectBizType) {
            case ZZ:
            case ZL:
                clientIds = JSONArray.parseArray(projReviewBaseInfo.getLesseeInfo()).toJavaList(ClientInfo.class).stream().map(ClientInfo::getClientId).collect(Collectors.toList());
                break;
            case BL:
            case ZR:
                clientIds = JSONArray.parseArray(projReviewBaseInfo.getCreditorInfo()).toJavaList(ClientInfo.class).stream().map(ClientInfo::getClientId).collect(Collectors.toList());
                break;
        }
        if (ObjectUtil.isNotEmpty(projReviewBaseInfo.getGuaranteeInfo())) {
            clientIds.addAll(JSONArray.parseArray(projReviewBaseInfo.getGuaranteeInfo()).toJavaList(ClientInfo.class).stream().map(ClientInfo::getClientId).collect(Collectors.toList()));
        }
        return clientIds;
    }
