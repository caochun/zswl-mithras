package cn.zswltech.mithras.service.service.archives;
...
import cn.zswltech.mithras.service.enums.archives.ArchiveTemplateStatusEnum;
import cn.zswltech.mithras.service.enums.archives.ArchivesFlowStatusEnum;
import cn.zswltech.mithras.service.enums.archives.ArchivesStatusEnum;
...
import cn.zswltech.mithras.service.mapper.archives.*;
import cn.zswltech.mithras.service.mapper.model.MaterialsList;
import cn.zswltech.mithras.service.mapper.model.archives.*;
...
public class ArchivesManageService extends ServiceImpl<ArchivesManagementMapper, ArchivesManagement> {
    ...
    public List<ArchivesUploadSelectRsp> archivesUploadInfo(ArchivesInfoREQ req){
        ...
        List<MaterialsList> materialsLists = materialsListService.listBy(BusinessModuleEnum.ARCHIVES.name(), management.getId());
    ...
    public Void archivesEffect(ArchivesInfoREQ req) {
        ...
        if (management.getType().equals(0)) {
            ...
            List<MaterialsList> list = materialsListService.listBy(BusinessModuleEnum.ARCHIVES.name(), management.getId());
            if (CollectionUtil.isEmpty(list)){
                throw new MithrasException("无归档数据,无法审批");
            }

        }
        if(isInProcess(req.getId(), BusinessModuleEnum.ARCHIVES)){
    ...
    public ArchiveDownloadFlowRSP downloadFlowInfo(ArchivesFlowInfoREQ req) {
        ...
        List<MaterialsList> files = materialsListService.getByIds(fileIds);
