...
import cn.zswltech.mithras.factory.feign.RatingManagementClient;
...
import cn.zswltech.mithras.service.mapper.budget.EclExecutePredictBaseInfoMapper;
...
public class EclExecutePredictBaseInfoService extends ServiceImpl<EclExecutePredictBaseInfoMapper, EclExecutePredictBaseInfo> {
    ...
    public Long create(EclExecutePredictBaseInfoAddREQ req) {
        ...
        if(ObjectUtil.isNotEmpty(eclExecuteRecords)) {
           ...
           /* Map<String, LocalDate> maxOverdueDay = getMaxOverdueDay(contractCodes);
            ...
            eclExecutePredictRecordService.saveBatch(addRecords);
        }
        //调用计算方法
    ...
    public void removeByBudgetPlan(Long budgetPlanId) {
        ...
        this.update(lambdaUpdateWrapper);
        //删除子数据
        eclExecutePredictRecordService.removeByExecutePredictId(budgetPlanId);
    }
    ...
    public void calculation(Long eclExecutePredictBaseInfoId, List<Long> executePredictRecordList) {
        ...
        try {
            //获取详情数据
            List<EclExecutePredictRecord> executePredictRecords = eclExecutePredictRecordService.list(Wrappers.<EclExecutePredictRecord>lambdaQuery()
                    .eq(EclExecutePredictRecord::getExecutePredictId, eclExecutePredictBaseInfoId)
                    .in(ObjectUtil.isNotEmpty(executePredictRecordList), EclExecutePredictRecord::getId, executePredictRecordList));
            //转模型入参
            ...
            //更新记录
            eclExecutePredictRecordService.updateBatchById(executePredictRecords);
            //通知预算更新
            //先按照计算月份分区
