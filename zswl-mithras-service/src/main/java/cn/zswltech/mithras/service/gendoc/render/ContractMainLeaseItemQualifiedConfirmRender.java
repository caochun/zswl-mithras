 ...
 * @author wangchuanhao
 * @date 2023/2/15
 * @description 租赁合同-租赁物合格接收确认书
 */
@Component
public class ContractMainLeaseItemQualifiedConfirmRender extends AbstractContractRender<ContractBaseInfo> {
    @Override
    public String render(OutputStream outputStream, ContractBaseInfo contractBaseInfo) throws Exception {
        ...
        if (contractTenantryMap.size() == 1) {
            inputStream = getBean(FileTemplateService.class).getTemplate("合同-租赁合同", "合同_租赁合同_附属_租赁物合格接收确认书_单一承租人.docx");
        } else {
            inputStream = getBean(FileTemplateService.class).getTemplate("合同-租赁合同", "合同_租赁合同_附属_租赁物合格接收确认书_共同承租人.docx");
        ...
        return "1-" + ContractSubTypeEnum.LEASE_ITEM_QUALIFIED_CONFIRM.getSort() + "." + ContractSubTypeEnum.LEASE_ITEM_QUALIFIED_CONFIRM.getDisplay() + GlobalConstants.OFFICE_WORD_SUFFIX;
    }
    ...
    private FileTemplate getFileTemplate(ContractBaseInfo contractBaseInfo) {
        ...
        if (contractTenantryMap.size() == 1) {
            fileTemplate = getBean(FileTemplateService.class).getTemplateRecord("合同-租赁合同", "合同_租赁合同_附属_租赁物合格接收确认书_单一承租人.docx");
        } else {
            fileTemplate = getBean(FileTemplateService.class).getTemplateRecord("合同-租赁合同", "合同_租赁合同_附属_租赁物合格接收确认书_共同承租人.docx");
        }
        return fileTemplate;
    }
