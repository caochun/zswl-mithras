...
@Slf4j
@Service
public class BudgetExamineBenefitService extends ServiceImpl<BudgetExamineBenefitMapper, BudgetExamineBenefit> {
    ...
    public void add(BudgetExamineBenefitAddREQ req) {
        ...
        // 项目利润表
        List<FinanceProjectProfitDetail> projectProfitDetails = financeProjectProfitDetailService.list(Wrappers.<FinanceProjectProfitDetail>lambdaQuery()
        ...
        for (Map.Entry<Long, List<BudgetExamineBenefit>> entry : todoMap.entrySet()) {
            ...
            long incomeTax = Util.mithrasLongDecimalTwo(incomeTaxBD.longValue());
        ...
        todoMap.get(optional.get().getId()).stream().filter(e -> Objects.equals(e.getFieldName(), BudgetExamineBenefitEnum.INCOME_TAX.name())).findAny().ifPresent(budgetExamineBenefit -> budgetExamineBenefit.setFieldValue(Util.mithrasLongDecimalTwo(gglrzxIncomeTaxBD.longValue())));
        // 保存
    ...
    public void refreshProcessData(Long budgetExamineId) {
        ...
        for (Map.Entry<Long, List<BudgetExamineBenefit>> entry : map.entrySet()) {
            ...
            long tax = Util.mithrasLongDecimalTwo(incomeTaxBD.longValue());
        ...
        if (CollectionUtil.isNotEmpty(toUpdateList)) {
            ...
            if (Objects.nonNull(gglrzxIncomeTaxBenefit)) {
                BigDecimal gglrzxIncomeTaxBD = assessmentProfitSumBD.multiply(BigDecimal.valueOf(0.25)).subtract(excludeGGLRZXIncomeTaxSumBD);
                gglrzxIncomeTaxBenefit.setFieldValue(Util.mithrasLongDecimalTwo(gglrzxIncomeTaxBD.longValue()));
    ...
    private BudgetExamineBenefitListRSP convertToBudgetExamineBenefitListRSP(Long deptId, String deptName, int sort, List<BudgetExamineBenefit> dataList) {
        ...
        for (BudgetExamineBenefit benefit : dataList) {
            ...
            if (StrUtil.equalsAny(benefit.getFieldName(), BudgetExamineBenefitEnum.BUSINESS_INVESTMENT_SCALE.name(), BudgetExamineBenefitEnum.END_OF_MONTH_ASSET_TOTAL.name())) {
                ...
                benefit.setFieldValue(Util.mithrasLongDecimalTwo(b.longValue()));
            }
            // 分组
