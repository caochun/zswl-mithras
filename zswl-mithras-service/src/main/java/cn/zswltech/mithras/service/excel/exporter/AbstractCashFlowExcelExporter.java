...
public abstract class AbstractCashFlowExcelExporter<T> {
    ...
    @Resource
    protected PaymentActualDetailService paymentActualDetailService;
        ...
        if (Objects.isNull(receiptId)) {
            ...
            if (CollectionUtil.isEmpty(paymentBaseInfoList)) {
                ...
                if (CollectionUtil.isEmpty(paymentActualDetailList)) {
                    ...
                    List<PaymentActualDetail> filterList = paymentActualDetailList.stream()
                            .filter(item -> Objects.nonNull(item.getPaidInDate()))
                            .sorted(Comparator.comparing(PaymentActualDetail::getPaidInDate))
                            .collect(Collectors.toList());
                    PaymentActualDetail paymentActualDetail = filterList.get(0);
