...
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.dto.BudgetPlanPayDetailIncomeGroupDTO;
...
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.BudgetPlanPayDetailIncomeSharingMapper;
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.model.BudgetPlanPayDetailIncomeSharing;
...
@Slf4j
@Service
public class BudgetPlanPayDetailIncomeSharingService extends ServiceImpl<BudgetPlanPayDetailIncomeSharingMapper, BudgetPlanPayDetailIncomeSharing> {
    @Resource
    ...
    public BudgetPlanPayDetailDynamicTableRSP getIncomeSharingDynamicTable(Long budgetPlanPayDetailId) {
        BudgetPlanPayDetail budgetPlanPayDetail = budgetPlanPayDetailService.getById(budgetPlanPayDetailId);
...
//            sumYearRow.add("年小计");
            ...
            for (int i = 0; i < allYears.size(); i++) {
                ...
                if (i == 0 && Objects.nonNull(budgetPlanPayDetailPrice) && Objects.nonNull(budgetPlanPayDetailPrice.getProjectAmount()) && Objects.nonNull(budgetPlanPayDetailPrice.getConsultingFeeRate())) {
                    ...
                    consultingFee = Util.mithrasLongDecimalTwo(consultingFeeBD.longValue());
                    BigDecimal consultingFeeWithoutTaxDB = consultingFeeBD.divide(BigDecimal.ONE.add(FinancialUtil.ensureConsultingTaxRate()), 20, RoundingMode.HALF_UP);
                    consultingFeeWithoutTax = Util.mithrasLongDecimalTwo(consultingFeeWithoutTaxDB.longValue());
                }
                ...
                BigDecimal sumWithoutTaxBD = new BigDecimal(sum).divide(BigDecimal.ONE.add(FinancialUtil.ensureValueAddedTaxRate(budgetPlanPayDetail.getLeaseType())), 20, RoundingMode.HALF_UP);
                long sumWithoutTax = Util.mithrasLongDecimalTwo(sumWithoutTaxBD.longValue());
