...
public class OauthController implements OAuthApi {
    ...
    public R<String> authorize(@RequestHeader("token") String token) {
        try {
            ...
            redisService.set(code, qjtAc, TOKEN_EXPIRE_TIME);
            return R.ok(code);
    ...
    public Map<String, String> token( Map<String, String> param) {
        ...
        redirect_uri = redirect_uri + "?access_token=" + redisService.get(codeList);
        Map<String, String> map = new HashMap();
        map.put("access_token", redisService.get(codeList));
        map.put("token_type", "Bearer");
        map.put("expires_in", String.valueOf(TOKEN_EXPIRE_TIME));
        map.put("refresh_token", redisService.get(codeList));
        return map;
    }
