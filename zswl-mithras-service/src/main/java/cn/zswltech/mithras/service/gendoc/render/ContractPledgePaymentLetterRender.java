 ...
 */
@Component
public class ContractPledgePaymentLetterRender extends AbstractContractRender<ContractPledge> {

    @Override
    public String render(OutputStream outputStream, ContractPledge contractPledge) throws Exception {
        InputStream inputStream = SpringUtil.getBean(FileTemplateService.class).getTemplate("合同-质押合同", "5-1.指示付款函.docx");
    ...
    protected boolean customShowFile(ContractPledge contractPledge) {
        FileTemplate templateRecord = SpringUtil.getBean(FileTemplateService.class).getTemplateRecord("合同-质押合同", "5-1.指示付款函.docx");
