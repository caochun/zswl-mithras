...
public class TycMortgageInfoConvert {
    ...
    public static TycMortgageInfo tycResp2Entity(TycMortgageInfoResp.ItemsDTO resp) {
        ...
        tycMortgageInfo.setCancelDate(Optional.ofNullable(baseInfoDTO.getCancelDate()).map(DateUtil::timestamp2LDT).orElse(null));
        tycMortgageInfo.setPublishDate(Optional.ofNullable(baseInfoDTO.getPublishDate()).map(DateUtil::timestamp2LDT).orElse(null));
