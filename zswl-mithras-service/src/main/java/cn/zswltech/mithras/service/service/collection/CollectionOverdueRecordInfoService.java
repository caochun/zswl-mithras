...
//    }
    ...
    public void doRerunPenaltyInterest(List<Long> allCollectionIds) {
        ...
        //查询核销明细
        Map<Long, List<CollectionRecordInfo>> collectId2Record = collectionRecordInfoService.listByCollectionIds(collectionIds).stream().collect(Collectors.groupingBy(CollectionRecordInfo::getCollectionId));
        //构建新罚息记录
        ...
        collectionBaseInfos.forEach(collectionBaseInfo -> {
            ...
            for (LocalDate date = collectionBaseInfo.getPlanCollectionDate(); date.isBefore(LocalDate.now()); date = date.plusDays(1)) {
                ...
                BigDecimal nowAmount = Util.mithrasInteger2BigDecimal(rate).multiply(Util.mithrasLong2BigDecimal(amount)).multiply(BigDecimal.valueOf(100L)).setScale(10, BigDecimal.ROUND_HALF_UP);
                ...
                recordInfo.setLastPenaltyInterest(Util.mithrasLongDecimalTwo(tempAmount.longValue()));
                newCollectionOverdueRecord.add(recordInfo);
            }
            //更新收款
            collectionBaseInfo.setPenaltyInterest(Util.mithrasLongDecimalTwo(tempAmount.longValue()) - LongUtil.null2zero(collectionBaseInfo.getPenaltyInterestDeductionAmount()));
        });
        //插入数据，更新数据
        //删除罚息记录
        ...
        if (!newCollectionOverdueRecord.isEmpty()) {
            SpringContextHolder.getBean(CollectionOverdueRecordInfoService.class).saveBatch(newCollectionOverdueRecord);
        }
        SpringContextHolder.getBean(CollectionBaseInfoService.class).updateBatchById(collectionBaseInfos);
    }
