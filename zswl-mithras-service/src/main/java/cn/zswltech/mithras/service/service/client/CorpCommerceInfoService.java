...
import cn.zswltech.mithras.customer.domain.enums.client.OwnershipTypeEnum;
...
//        setClientAuthority(newCorpCommerceInfo, existClient, startUserId);
        ...
        //公海用户修改类型
        if (RiskControlIndustryClassify.INTRA_GROUP_COLLABORATION.name().equals(newCorpCommerceInfo.getRiskControlIndustryClassify())) {
...
//        clientMapper.updateAnnotationIncludeNullById(existClient);
    ...
    public void modify(CorpCommerceInfoModifyREQ req) {
        ...
        if (Objects.nonNull(originalInfo) && Objects.equals(existClient.getClientStatus(), ClientStatus.TAKE_EFFECT.name())) {
            ...
            boolean oldData = Objects.equals(originalInfo.getRiskControlIndustryClassify(), RiskControlIndustryClassify.INTRA_GROUP_COLLABORATION.name());
            boolean newData = !Objects.equals(newCorpCommerceInfo.getRiskControlIndustryClassify(), RiskControlIndustryClassify.INTRA_GROUP_COLLABORATION.name());
            if (oldData == newData) {
...
//        }
    ...
    public ClientCorpCommerceInfoValidRSP valid(ClientCorpCommerceInfoValidREQ req) {
        ...
        CorpCommerceInfo corpCommerceInfo = SpringUtil.getBean(CorpCommerceInfoMapper.class).selectOne(Wrappers.<CorpCommerceInfo>lambdaQuery()
                .eq(CorpCommerceInfo::getClientId, req.getClientId()));
        List<CorpShareholderInfo> corpShareholderInfos = SpringUtil.getBean(CorpShareholderInfoMapper.class).selectList(Wrappers.<CorpShareholderInfo>lambdaQuery()
                .eq(CorpShareholderInfo::getClientId, req.getClientId()));
        // 下面的接口设置超时机制
        CompletableFuture<Map<Long, ClientBusinessHistoryBO>> mapCompletableFuture = CompletableFuture.supplyAsync(() -> clientService.compareBusiness(ListUtil.toList(req.getClientId())));
        ...
        ContractCompareBusinessRSP rsp = contractBaseInfoService.buildContractCompareBusinessRSP(longClientBusinessHistoryBoMap.get(req.getClientId()), client, corpCommerceInfo,
