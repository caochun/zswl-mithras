...
import cn.zswltech.mithras.service.mapper.ProfitCalculateResultMapper;
...
@Slf4j
@Service
public class ProfitCalculateResultService extends ServiceImpl<ProfitCalculateResultMapper, ProfitCalculateResult> {
    @Resource
    ...
    public ProfitCalculateResult calculate(Long contractId) {
        ...
        // 找到投放时间
        PaymentActualDetail paymentActualDetail = paymentActualDetailService.getEarliestPayment(contractId);
        ...
        toSaveModel.setAdditionalTax(Util.mithrasLongDecimalTwo(additionalTax.longValue()));
        ...
        // 利润总额（扣除费用） = 利润总额 * 0.8
        toSaveModel.setProfitExcludeFee(Util.mithrasLongDecimalTwo(BigDecimal.valueOf(toSaveModel.getProfit()).multiply(BigDecimal.valueOf(0.8)).setScale(2, RoundingMode.HALF_UP).longValue()));
        // 年末剩余本金
    ...
    public ProfitCalculateResultListRSP pageList(ProfitCalculateResultListREQ req) {
        ...
        List<ProfitCalculateResultListRSP.Data> rspList = dbList.stream().map(item -> {
            ...
            if (StrUtil.isNotBlank(item.getRiskControlIndustryClassify())) {
                rspData.setProjectClassify(kpiProjectDistributionBaseInfoService.ensureProjClassify(item.getRiskControlIndustryClassify()).display());
            }
            // 处理业务类型
    ...
    public List<ProfitCalculateExcelModel> listExcelModel(ProfitCalculateResultListREQ req) {
        ProfitCalculateResultListRSP rsp = this.pageList(req);
    ...
    private Long ensureConfirmIncomeThisYear(ContractBaseInfoLib contractBaseInfoLib) {
        FinanceProjectProfitDetail financeProjectProfitDetail = financeProjectProfitDetailService.getLatestOneByContractId(contractBaseInfoLib.getOriginId());
    ...
    private Long ensureCalculateInterestThisYear(ContractBaseInfoLib contractBaseInfoLib, BigDecimal taxRate, List<ContractRentActualLib> contractRentActualLibList) {
        ...
        return Util.mithrasLongDecimalTwo(interest.divide(BigDecimal.ONE.add(taxRate), 2, RoundingMode.HALF_UP).longValue());
    }
    ...
    private Long calculateFtpInterestSingleReceipt(ContractReceiptLib contractReceiptLib, List<ContractRentActualLib> contractRentActualLibList) {
        // 获取当前已有FTP计息数据
        FtpInterestBaseInfo ftpInterestBaseInfo = ftpInterestBaseInfoService.getOneByReceiptId(contractReceiptLib.getOriginId());
        ...
        FtpInterestDetailRecord latestDetailRecord = ftpInterestDetailRecordService.getLatestRecord(ftpInterestBaseInfo.getId(), now);
        ...
        // 查询所有收到的票据
        List<BillManagement> billManagementList = billManagementService.listByReceiptId(contractReceiptLib.getOriginId());
        // 按日期存到期票据金额
        ...
        while (!targetDate.isAfter(endDate)) {
            ...
            if (i > 0) {
                currentReceiptResult = currentReceiptResult + Util.mithrasLongDecimalTwo(i);
            }
    ...
    private Long ensureRiskBalanceEndOfLastYear(ContractBaseInfoLib contractBaseInfoLib) {
        ...
        query.eq(FinanceProjectProfitDetail::getContractId, contractBaseInfoLib.getOriginId());
        query.eq(FinanceProjectProfitDetail::getYear, LocalDate.now().getYear() - 1);
        query.eq(FinanceProjectProfitDetail::getMonth, 12);
        FinanceProjectProfitDetail projectProfitDetail = financeProjectProfitDetailService.getOne(query);
    ...
    private Long calculateRemainingEarnestEndOfThisYearSingleReceipt(ContractReceiptLib contractReceiptLib) {
        ...
        if (Objects.nonNull(marginBaseInfo) && Objects.nonNull(marginBaseInfo.getCollectionAmount())) {
            // 按照付款比例进行拆分
            List<PaymentBaseInfo> allPayment = paymentBaseInfoService.listEffectPaymentByContractId(contractReceiptLib.getContractId());
            ...
            return Util.mithrasLongDecimalTwo((long) (marginBaseInfo.getCollectionAmount() * (currentReceipt / total)));
        }
        return 0L;
    }
