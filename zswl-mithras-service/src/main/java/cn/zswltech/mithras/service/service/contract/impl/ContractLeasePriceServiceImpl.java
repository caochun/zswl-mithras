...
public class ContractLeasePriceServiceImpl extends ServiceImpl<ContractLeasePriceMapper, ContractLeasePrice> implements ContractLeasePriceService {
    ...
    private void checkModify(ContractLeasePriceModifyREQ req) {
        ContractLeasePrice leasePrice = baseMapper.selectById(req.getId());
        ContractBaseInfo baseInfo = baseInfoMapper.selectById(leasePrice.getContractId());
        //合同金额
        /*if(ObjectUtil.isNotEmpty(req.getApplyCreditAmount())){
            if(req.getApplyCreditAmount() <= 0 || req.getApplyCreditAmount() > baseInfo.getRemainAvailableQuota()){
                throw new MithrasException("合同金额需大于0且小于等于可用金额");
            }
        }*/
//        //租赁期限(月)
//        if(ObjectUtil.isNotEmpty(req.getLeaseMonthCount()) && Objects.nonNull(leasePrice.getProjLeaseMonthCount())){
//            if(req.getLeaseMonthCount() <= 0 || req.getLeaseMonthCount() > leasePrice.getProjLeaseMonthCount()){
//                throw new MithrasException("租赁期限必须大于0，且小于等于项目评审中的期限");
//            }
//        }
        //  校验手续费和首期利息
        ...
        /*if(ObjectUtil.isNotEmpty(req.getLprPercent())){
            if((req.getLprPercent() + req.getLprAddPercent()) < leasePrice.getProjIrrPercent()){
                throw new MithrasException("租赁利率必须大于等于项目中的IRR");
            }
        }*/
...
//        if(ObjectUtil.isNotEmpty(req.getIrrPercent()) && Objects.nonNull(leasePrice.getProjIrrPercent())){
//            if(req.getIrrPercent() < leasePrice.getProjIrrPercent()){
