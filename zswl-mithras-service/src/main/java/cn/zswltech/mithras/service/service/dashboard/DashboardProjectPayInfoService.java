...
import cn.zswltech.mithras.dashboard.infrastructure.persistence.mapper.model.DashboardProjectPayInfoQuery;
...
@Slf4j
@Service
public class DashboardProjectPayInfoService extends DashboardProjectService implements cn.zswltech.mithras.dashboard.application.DashboardProjectPayApplicationService {
    @Resource
    ...
    public List<DashboardProjectPayListRSP> list(DashboardProjectPayListREQ req) {
        ...
        return this.buildRspList(dbList, dbResult -> {
            ...
            if (Objects.nonNull(dbResult.getActualPayAmount())) {
                rsp.setActualPayAmount(new ValueUnitDTO(Util.toYuanWithoutSplit(dbResult.getActualPayAmount()), "元"));
            }
            ...
            if (Objects.nonNull(dbResult.getEarnestMoney())) {
                rsp.setEarnest(req.getQueryDimension().equals(PayInfoQueryDimensionEnum.CONTRACT.name()) ?
                        new ValueUnitDTO(Util.toYuanWithoutSplit(dbResult.getEarnestMoney()), "元") :
                        new ValueUnitDTO(Util.toYuanWithoutSplit(dbResult.getPaymentEarnestMoney()), "元"));
            }
