...
import cn.zswltech.mithras.service.convert.riskcontrol.RiskControlConcentrationClientConverter;
...
import cn.zswltech.mithras.service.mapper.riskcontrol.RiskControlConcentrationClientMapper;
...
import static cn.zswltech.gruul.common.constant.OrgConstants.BUSINESS_DEPT;
...
public class RiskControlConcentrationClientService
    ...
    @Resource
    private RiskControlStrategyService strategyService;
    @Resource
    ...
    public void riskControlConcentrationClientJobHandler() {
        try {
            ...
            Map<Long, Long> clientIdToRemaining =
                    remainingPrincipalServiceImpl.remainingPrincipalGroupByClientId(remainingPrincipalQueryDto);
            // 计算所有存在逾期的客户的逾期金额
            Map<Long, Long> clientIdToOverdueAmount = remainingPrincipalServiceImpl.overdueAmountGroupByClient();
            // 计算所有客户的保证金余额
            Map<Long, Long> clientIdToMargin = getBean(MarginBaseInfoService.class).getClientMarginBalances(clientsMap.keySet());
            //获取客户信息
            Map<Long, Long> clientSponsorMap = getBean(ClientService.class).listByIds(clientsMap.keySet()).stream().filter(e -> Objects.nonNull(e.getBelongSponsorId())).collect(Collectors.toMap(Client::getId, Client::getBelongSponsorId));
            // 查询当月数据
    ...
    public PageR<RiskControlConcentrationClientListRSP> listClient(RiskControlConcentrationClientListREQ req) {
        ...
        Set<Long> bizUserIds = bizUserIds();
