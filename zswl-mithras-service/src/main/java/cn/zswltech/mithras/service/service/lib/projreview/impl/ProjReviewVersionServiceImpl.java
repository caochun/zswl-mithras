    ...
    public void recordPricingVersion(Long mainId, VersionTypeEnum type, Long operatorId, String processInstanceId) {
        ...
        if (ObjectUtil.isEmpty(oleCommonVersion)) {
            CommonVersion newCommonVersion = CommonVersion.builder()
                    .mainId(mainId)
                    .type(type.getType())
                    .module(BusinessModuleEnum.PROJ_REVIEW.name())
...
//                .eq(CommonVersion::getFlowType, "pricing")
    ...
    public Map<String, DiffValue> projReviewPricingBaseInfoCompare(ProjReviewCompareREQ req) {
        ...
        if (pricingBaseInfoLib != null) {
            ProjPricingBaseInfoDetailRSP pricingDetailRSP = SpringContextHolder.getBean(ProjPricingBaseInfoLibService.class).detail(pricingBaseInfo.getId(), pricingBaseInfoLib.getVersion());
            ProjReviewBaseInfoDetailRSP baseInfoRSP = SpringContextHolder.getBean(ProjReviewBaseInfoService.class).detail(projReviewBaseInfo.getId(), null);
    ...
    public Map<String, DiffValue> projReviewPricingPriceCompare(ProjReviewCompareREQ req) {
        ...
        if (PricingBaseInfoLib != null) {
            ...
            ProjPricingPriceDetailRSP projPricingPriceDetailRSP = SpringContextHolder.getBean(ProjPricingPriceService.class).oldDetail(projId);
            if (ObjectUtil.isNotEmpty(projPricingPriceDetailRSP)) {
                if (ObjectUtil.isNotEmpty(projPricingPriceDetailRSP.getAocPriceDetailRSP())) {
                    ProjReviewAocPrice reviewAocPrice = SpringContextHolder.getBean(ProjReviewAocPriceService.class).getByProjectId(req.getId());
                ...
                if (ObjectUtil.isNotEmpty(projPricingPriceDetailRSP.getLeasePriceDetailRSP())) {
                    ProjReviewLeasePrice reviewLeasePrice = SpringContextHolder.getBean(ProjReviewLeasePriceService.class).getByProjectId(req.getId());
                ...
                if (ObjectUtil.isNotEmpty(projPricingPriceDetailRSP.getFactoringPriceDetailRSP())) {
                    ProjReviewFactoringPrice reviewFactoringPrice = SpringContextHolder.getBean(ProjReviewFactoringPriceService.class).getByProjectId(req.getId());
        ...
        if (pricingBaseInfoLib != null) {
            ...
            if(lastVersion != null) {
                ...
                if (CollectionUtils.isNotEmpty(pricingCashFlowPlanLibList)) {
                    // 本次变更定价的现金流
                    List<ProjReviewCashFlowPlan> reviewCashFlowPlanList = reviewCashFlowPlanService.listByProjReviewId(projReviewBaseInfo.getId(), null);
