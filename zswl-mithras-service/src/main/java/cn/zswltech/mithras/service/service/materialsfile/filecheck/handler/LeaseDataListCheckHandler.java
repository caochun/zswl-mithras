...
@Component
public class LeaseDataListCheckHandler extends FileModuleCheck {
...
//            }
    ...
    private void checkAuth(String materialsType) {
        ...
        if (StrUtil.equalsAny(materialsType, LeaseFileTypeEnums.OPERATION_MANAGER_UPLOAD_OWNERSHIP.name(), LeaseFileTypeEnums.OPERATION_MANAGER_UPLOAD_DUPLICATE_CHECK.name() , LeaseFileTypeEnums.OPERATION_MANAGER_UPLOAD_OTHER.name())) {
            if (!sysUserService.currentUserIsSpecificJob(JobEnum.operationManagement.name()) && !sysUserService.currentUserIsSpecificJob(JobEnum.operationManagementReview.name())) {
                throw new AuthCheckException("非运营经理,不可操作");
            }
            ...
            if (!sysUserService.currentUserIsSpecificJob(JobEnum.projmanager.name()) &&
                    (!sysUserService.currentUserIsSpecificJob(JobEnum.operationManagement.name()) && !sysUserService.currentUserIsSpecificJob(JobEnum.operationManagementReview.name()))) {
                throw new AuthCheckException("非项目经理,不可操作");
            }
        }
    }
}
