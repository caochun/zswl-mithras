 ...
 * @author dingqi
 * @date 2023/4/19
 * @description 租赁-直租-主合同-补足租赁保证金通知书
 */
@Component
public class ContractZLZZSuppleEarnestRender extends AbstractContractZLZZRender<ContractBaseInfo> {
    @Override
    public String render(OutputStream outputStream, ContractBaseInfo contractBaseInfo) throws Exception {
        ...
        // 渲染文档
        InputStream inputStream = getBean(FileTemplateService.class).getTemplate("合同-直租合同", "1-8.补足租赁保证金通知书.docx");
        XWPFTemplate template = XWPFTemplate.compile(inputStream).render(renderMap);
        template.writeAndClose(outputStream);
        return ContractSubTypeEnum.ZL_ZZ_SUPPLE_EARNEST.display() + GlobalConstants.OFFICE_WORD_SUFFIX;
    }
    ...
    protected boolean customShowFile(ContractBaseInfo contractBaseInfo) {
        FileTemplate templateRecord = getBean(FileTemplateService.class).getTemplateRecord("合同-直租合同", "1-8.补足租赁保证金通知书.docx");
