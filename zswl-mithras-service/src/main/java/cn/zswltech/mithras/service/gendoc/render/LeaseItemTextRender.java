...
import cn.zswltech.mithras.system.service.LeaseItemCommonService;
 ...
 */
@Component
public class LeaseItemTextRender extends AbstractBasicRender<LeaseItemInfo> {
    @Resource
    ...
    @Override
    public String render(OutputStream outputStream, LeaseItemInfo leaseItemInfo) throws Exception {
        ...
        if (Objects.isNull(projReviewBaseInfo)) {
            throw new MithrasException("项目评审信息不存在，生成租赁物清单文件失败");
        }
        Client client = businessDataRepository.getClient(projReviewBaseInfo.getClientId());
        if (Objects.isNull(client)) {
            throw new MithrasException("客户信息不存在，生成租赁物清单文件失败");
        }
        Map<String, Object> renderMap = new HashMap<>();
        renderMap.put("mainLesseeName", client.getClientName());
        List<String> headerList = null;
        if (StrUtil.isNotBlank(leaseItemInfo.getItemListHeader())) {
            headerList = JSONUtil.toList(leaseItemInfo.getItemListHeader(), String.class);
        }
        List<LeaseItemListRowData> leaseItemListRowData = leaseItemListRowDataService.listByLeaseItemInfoId(leaseItemInfo.getId());
        if (CollectionUtil.isEmpty(headerList) || CollectionUtil.isEmpty(leaseItemListRowData)) {
            throw new MithrasException("数据缺失，生成租赁物清单文件失败");
        }
        renderMap.put("leaseItemTable", this.renderLeaseItemTable(leaseItemCommonService.preHandleHeader(headerList), leaseItemListRowData));
        InputStream inputStream = getBean(FileTemplateService.class).getTemplate("租赁物管理-租赁物清单", "租赁物管理_租赁物文本_租赁物清单.docx");
        // 渲染文档
        XWPFTemplate template = XWPFTemplate.compile(inputStream).render(renderMap);
        template.writeAndClose(outputStream);
        return "租赁物清单" + GlobalConstants.OFFICE_WORD_SUFFIX;
    }

    private TableRenderData renderLeaseItemTable(List<String> headerList, List<LeaseItemListRowData> infoList) {
        // 确定表头
        Set<String> show = new HashSet<>();
        show.add("序号");
        for (LeaseItemListRowData leaseItemListRowData : infoList) {
            if (StrUtil.isBlank(leaseItemListRowData.getRowData())) {
                continue;
            }
        ...
        for (int i = 0; i < infoList.size(); i++) {
            LeaseItemListRowData leaseItemListRowData = infoList.get(i);
            if (StrUtil.isBlank(leaseItemListRowData.getRowData())) {
                continue;
            }
            Map<String, Object> cellMap = JSONUtil.toBean(leaseItemListRowData.getRowData().replace("\n", "\\n"), Map.class);
