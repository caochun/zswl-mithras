...
public class AssociationExternFinancingData extends AbstractDataStore<AssociationExternalFinancing> {
    ...
    private List<AssociationExternalFinancing> indirect(LocalDate targetDate) {
        ...
        List<FundFinancingBaseInfo> todoList = SpringUtil.getBean(FundFinancingBaseInfoService.class).list(query);
    ...
    private AssociationExternalFinancing buildFromIndirect(Map<String, String> dictNameMap, FundFinancingBaseInfo fundFinancingBaseInfo, LocalDate targetDate) {
        ...
        List<FundReceiptFlowDetail> flowDetailList = SpringUtil.getBean(FundReceiptFlowDetailService.class).listByReceiptRepayId(fundReceiptRepayBaseInfo.getId());
    ...
    private List<AssociationExternalFinancing> direct(LocalDate targetDate) {
        ...
        List<FundDirectFinancingBaseInfo> todoList = SpringUtil.getBean(FundDirectFinancingBaseInfoService.class).list(query);
        if (CollectionUtil.isEmpty(todoList)) {
            return Collections.emptyList();
        }
        List<AssociationExternalFinancing> result = new LinkedList<>();
        for (FundDirectFinancingBaseInfo fundDirectFinancingBaseInfo : todoList) {
            // 直融需要拆到产品明细层面
            List<FundDirectFinancingProductDetail> productDetailList = SpringUtil.getBean(FundDirectFinancingProductDetailService.class).listByFinancingId(fundDirectFinancingBaseInfo.getId());
    ...
    private AssociationExternalFinancing buildFromDirect(Map<String, String> dictNameMap, FundDirectFinancingBaseInfo fundDirectFinancingBaseInfo, FundDirectFinancingProductDetail productDetail, LocalDate targetDate) {
        ...
        // 需要分类型处理的取值逻辑
        List<FundDirectFinancingRepayActual> repayActualList = SpringUtil.getBean(FundDirectFinancingRepayActualService.class).listByFinancingId(fundDirectFinancingBaseInfo.getId());
        if (StrUtil.equalsAny(fundDirectFinancingBaseInfo.getDirectFinancingType(), DirectFinancingType.ABN.name(), DirectFinancingType.ABS.name())) {
            ...
            if (CollectionUtil.isNotEmpty(targetCashFlowCode)) {
                List<FundDirectFinancingRepayActualSplitRecord> repayActualSplitRecordList = SpringUtil.getBean(FundDirectFinancingRepayActualSplitRecordService.class).getProductRepayDetail(productDetail.getId(), FinancePaymentWriteOffOrderEnum.PRINCIPAL.name());
            ...
            instance.setLoanBal(Util.millimeterLong2WanBigDecimal(remainingPrincipal));
            // 到期日期
            List<FundDirectFinancingRepayActualSplit> repayActualSplitList = SpringUtil.getBean(FundDirectFinancingRepayActualSplitService.class).listByProductDetailId(productDetail.getId());
...
//            }
            ...
            // 寻找核销明细
            List<FundReceiptFlowDetail> detailList = SpringUtil.getBean(FundReceiptRepayBaseInfoService.class).getFinancingRepayDetail(fundDirectFinancingBaseInfo.getId(), FinancingTypeEnum.DIRECT.name());
            // 取到目标日期为止，后面的不要
