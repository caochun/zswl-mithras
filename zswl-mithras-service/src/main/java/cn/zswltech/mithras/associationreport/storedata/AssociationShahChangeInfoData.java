...
@Slf4j
@Component
public class AssociationShahChangeInfoData extends AbstractDataStore<AssociationShahChangeInfo> {
    @Override
    protected List<AssociationShahChangeInfo> parseFromExcel(InputStream inputStream) {
        List<List<Object>> rows = ExcelUtil.getReader(inputStream).read();
        if (CollectionUtil.isEmpty(rows)) {
            return Collections.emptyList();
        }
        log.info("金融局报送【股东股权信息一览表-股东变更记录】-Excel解析结果:{}", JSONUtil.toJsonStr(rows));
        // 校验是否符合导入模板
        if (rows.size() < 2) {
        ...
        boolean row1col1check = Objects.nonNull(row1col1) && StrUtil.equals(row1col1.toString(), "股东变更记录");
        boolean row2col1check = Objects.nonNull(row2col1) && StrUtil.equals(row2col1.toString(), "序号");
        if (!row1col1check || !row2col1check) {
            throw new MithrasException("<股东股权信息一览表-股东变更记录>导入文件格式有误，请下载系统模板进行导入");
        }
        // 查询一下字典，备用
        Map<String, Map<String, String>> dictNameMap = SpringUtil.getBean(AssociationDictionaryService.class).getDisplay2CodeMap();
        // 开始处理Excel数据
        List<AssociationShahChangeInfo> list = new LinkedList<>();
        for (int i = 2; i < rows.size(); i++) {
            ...
            try {
                list.add(this.convert(i-1, row, dictNameMap));
            } catch (Exception e) {
                log.error("金融局报送【股东股权信息一览表-股东变更记录】-第{}行数据处理异常", (i-1), e);
    ...
    @Override
    protected void check(List<AssociationShahChangeInfo> dataList) {
    ...
    @Override
    protected IService<AssociationShahChangeInfo> serviceBean() {
        return SpringUtil.getBean(AssociationShahChangeInfoService.class);
    }

    @Override
    protected AssociationReportCategoryEnum category() {
        return AssociationReportCategoryEnum.J0003;
    }

    private AssociationShahChangeInfo convert(int rowNum, List<Object> row, Map<String, Map<String, String>> dictNameMap) {
        AssociationShahChangeInfo bean = new AssociationShahChangeInfo();
        ...
        if (row.get(4) != null && StrUtil.isNotBlank(row.get(4).toString())) {
            ...
            if(shahGtoMode.equals("创设") || shahGtoMode.equals("受让")){ //todo 如果存在数据字典的话，需要提供数据字典
                bean.setShahGtoMode(shahGtoMode);
            }else{
                bean.setShahGtoMode(DICT_UNKNOWN_CODE);
            }
//            Map<String, String> contractTypeMap = dictNameMap.get(AssociationDictionaryCategoryEnum.PTY00021.name());//todo 需要提供数据字典
//            if (Objects.nonNull(contractTypeMap)) {
//                bean.setShahGtoMode(Optional.ofNullable(contractTypeMap.get(row.get(4).toString())).orElse(DICT_UNKNOWN_CODE));
//            } else {
//                bean.setShahGtoMode(DICT_UNKNOWN_CODE);
//            }
        }
...
//        if (row.get(7) != null) {
//            String storTranFlag = row.get(7).toString();
//            if(storTranFlag.equals("是") || storTranFlag.equals("否")){
//                bean.setStorTranFlag(storTranFlag);
//            }else{
...
//        }
        ...
        bean.setAltrShahFndrAmt(Optional.ofNullable(row.get(9)).filter(e -> NumberUtil.isNumber(e.toString())).map(e -> new BigDecimal(e.toString())).orElse(null));//变更后股东出资金额(万元)
        bean.setAltrHoldRati(Optional.ofNullable(row.get(10)).filter(e -> NumberUtil.isNumber(e.toString())).map(e -> new BigDecimal(e.toString())).orElse(null));//变更后持股比例
