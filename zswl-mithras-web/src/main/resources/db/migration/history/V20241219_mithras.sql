-- 经营全景视图优化开始 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
-- 机构表增加字段
alter table bifrost_org add column state tinyint(1) not null default 1 comment '状态，0-禁用，1-启用';
-- 订正数据
update bifrost_org set state = 0 where id in (7, 8, 10);
-- 经营全景视图优化结束 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
VALUES
    ('clientgrouplist', '客户管理-查询集团客户', 0, 6, NULL, NULL, NULL, 'POST', '/client/group/list', 1, NULL);
-- 集团客户查询优化结束 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
