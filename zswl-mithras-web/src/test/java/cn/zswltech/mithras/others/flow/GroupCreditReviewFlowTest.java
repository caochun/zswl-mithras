 ...
 */
public class GroupCreditReviewFlowTest extends FlowTest {

    @Test
    public void groupCreditReviewFlowTest() throws Exception {
        List<String> fileNameList = Arrays.asList("bpmn/授信评审创建流程.bpmn20.xml", "bpmn/授信评审修改流程.bpmn20.xml");
        for (String fileName : fileNameList) {
            ...
            UserTaskExt userTaskExtFinanceOfficer = UserTaskExt.builder()
                    ...
                    .parallelApprovalMethed(ParallelApprovalMethedEnum.ONE.getType())
                    .buttonList(Arrays.asList(ApprovalButtonTypeEnum.AGREE.name(), ApprovalButtonTypeEnum.COLLABORATE.name(), ApprovalButtonTypeEnum.BACK_TO_START_USER.name(), ApprovalButtonTypeEnum.WITHDRAW_TASK.name()))
                    .dynamicFormList(Arrays.asList(FlowDynamicFormEnum.projReview_setFinanceFile.name()))
                    .build();
            globalExt.getUserTaskExtMap().put(userTaskExtFinanceOfficer.getActivityId(), userTaskExtFinanceOfficer);

            // 财务总监
            UserTaskExt userTaskExtFinanceDirector = UserTaskExt.builder()
                    ...
                    .buttonList(Arrays.asList(ApprovalButtonTypeEnum.AGREE.name(), ApprovalButtonTypeEnum.COLLABORATE.name(), ApprovalButtonTypeEnum.BACK_TO_START_USER.name(), ApprovalButtonTypeEnum.WITHDRAW_TASK.name()))
                    .build();
            globalExt.getUserTaskExtMap().put(userTaskExtFinanceDirector.getActivityId(), userTaskExtFinanceDirector);
            ...
            UserTaskExt userTaskExtJurySecretaryCollect = UserTaskExt.builder()
                    ...
                    // 显示投票结果 上传会议纪要
                    .dynamicFormList(Arrays.asList(FlowDynamicFormEnum.projReview_showJudgesVotingResults.name(), FlowDynamicFormEnum.projReview_setMeetingFile.name(), FlowDynamicFormEnum.projReview_setMeetingRecord.name()))
                    .build();
