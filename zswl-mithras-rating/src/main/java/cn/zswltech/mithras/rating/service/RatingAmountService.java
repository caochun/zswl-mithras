...
import cn.zswltech.mithras.rating.application.port.RatingAmountClientFactPort;
...
import cn.zswltech.mithras.rating.versioning.ratingamount.impl.RatingAmountVersionServiceImpl;
...
//    }
    ...
    public ProcessResp findRelatedProcess(Long ratingClientId) {
        ...
        processPageReq.setModelKeyList(Arrays.asList(ProcessModelTypeEnum.RatingAmountCreateFlow.name(),
                ProcessModelTypeEnum.RatingAmountUpdateFlow.name()));
...
//                .isNull(RatingAmount::getAbandonTime));
        ...
        if (CollectionUtils.isNotEmpty(overdueAllClientList)) {
            Map<Long, Long> clientRemainingPrincipalMap = ratingClientSupportPort.getClientRemainingPrincipalMap(overdueAllClientList.stream().map(RatingClient::getClientId).collect(Collectors.toList()));
