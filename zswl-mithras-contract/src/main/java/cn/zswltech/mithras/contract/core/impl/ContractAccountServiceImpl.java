...
import cn.zswltech.mithras.dto.contract.account.ContractAccountAddREQ;
...
import cn.zswltech.mithras.dto.contract.account.ContractAccountModifyREQ;
import cn.zswltech.mithras.dto.contract.account.ContractAccountRemoveREQ;
...
public class ContractAccountServiceImpl extends ServiceImpl<ContractAccountMapper, ContractAccount> implements ContractAccountService {
    ...
    private void check(ContractAccount contractAccount) {
        ContractBaseInfo contractBaseInfo = Assert.notNull(contractBaseInfoMapper.selectById(contractAccount.getContractId()),
                () -> MithrasException.newException("合同信息不存在"));
        if (ProjectBizType.BL.name().equals(contractBaseInfo.getBizType()) || ProjectBizType.ZR.name().equals(contractBaseInfo.getBizType())) {
            if (Objects.equals(contractAccount.getAccountUse(), ContractAccountUseEnum.BLHK.name())
                    || Objects.equals(contractAccount.getAccountUse(), ContractAccountUseEnum.ZRHK.name())) {
