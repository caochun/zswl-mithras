package cn.zswltech.mithras.third.overduereport.infrastructure.client;
...
import cn.zswltech.mithras.third.overduereport.infrastructure.client.rsp.OverdueReportBaseRSP;
...
public abstract class OverdueReportApiHandler<T, F extends OverdueReportBaseRSP> implements PlatformApiHandler<T, F> {

    protected static final String DATA = "list";
    ...
    public F execute(T reqData) {
        ...
        try {
            ...
            // 构造请求地址
            log.info("OverdueReportApiHandler send cq url {}, requestData {}", url, requestData);
            // 根据请求类型发起请求
            try {
                ...
                log.info("OverdueReportApiHandler end http,url:{},time:{} error", url, e);
            }
            long endTime = System.currentTimeMillis();
            log.info("OverdueReportApiHandler end http,url:{},time:{}, responseData : {}", url, endTime - startTime, responseData);
        } catch (Exception e) {
            log.error("OverdueReportApiHandler connection fail, url:{}, request params:{}, error:{}", url, requestData, e);
        }
        F result = null;
    ...
    public boolean isExecuteSuccess(F resData) {
        ...
        if (ObjectUtil.isNotEmpty(resData)) {
            if (FinancialRSPENUM.SUCCESS.getResult().equals(resData.getState()) || ObjectUtil.equals(Boolean.TRUE, resData.getStatus())) {
                return Boolean.TRUE;
            } else {
                log.info("OverdueReportApiHandler {} execute error errorCode : {} message : {}", platformApi().apiName, resData.getErrorCode(), resData.getMessage());
            }
        }
    ...
    public String wrapRequestParam(T reqData) {
        if (PlatformApiEnum.OVERDUE_REPORT_APP_TOKEN.equals(platformApi()) || PlatformApiEnum.OVERDUE_REPORT_ACCESS_TOKEN.equals(platformApi())) {
