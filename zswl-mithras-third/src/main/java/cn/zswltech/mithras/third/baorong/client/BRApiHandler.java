package cn.zswltech.mithras.third.baorong.client;
...
import cn.zswltech.mithras.third.baorong.client.rsp.BRCommonRsp;
...
public abstract class BRApiHandler<T, F extends BRCommonRsp> implements PlatformApiHandler<T, F> {
    ...
    public F execute(T reqData) {
        ...
        try {
            ...
            // 构造请求地址
            log.info("BRApiHandler send cq url {}, requestData {}", url, requestData);
            // 根据请求类型发起请求
                ...
                try {
                    ...
                    log.info("BRApiHandler end http,url:{},time:{} error", url, e);
                }
            long endTime = System.currentTimeMillis();
            log.info("BRApiHandler end http,url:{},time:{}, responseData : {}", url, endTime - startTime, responseData);
        } catch (Exception e) {
            log.error("BRApiHandler connection fail, url:{}, request params:{}, error:{}", url, requestData, e);
        }
        F result = null;
    ...
    public boolean isExecuteSuccess(F resData) {
        // 权限验证成功
        if(ObjectUtil.isNotEmpty(resData) && ObjectUtil.isNotEmpty(resData.getCwgsHead())){
            if (YesOrNoNumberEnum.NO.getCode().equals(resData.getCwgsHead().getReturnCode())) {
                return Boolean.TRUE;
            } else {
                log.error("BRApiHandler {} execute error message : {}", platformApi().apiName, resData);
            }
        }
