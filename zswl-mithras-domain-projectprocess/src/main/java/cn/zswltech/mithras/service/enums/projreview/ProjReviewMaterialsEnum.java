...
//    BUSINESS_PRICING_APPROVAL_MEETING_REPORT("业务定价会议纪要", Collections.singletonList("userTask_financeOfficer_2"), 10),
    ...
    //现场尽调照片
    ON_SITE_DUE_DILIGENCE("现场尽调照片", new ArrayList<>(), 100),
    ;
    ...
    private static final Map<String, ProjReviewMaterialsEnum> map;

    static {
        map = Stream.of(ProjReviewMaterialsEnum.values()).collect(Collectors.toMap(ProjReviewMaterialsEnum::name, e -> e));
    }

    public static ProjReviewMaterialsEnum getByName(String name) {
        return map.get(name);
    }
...
//        return Arrays.asList(BUSINESS_PRICING_APPROVAL_FORM.name(), YIELD_REVIEW_REPORT.name(), GMO_MEETING_MINUTES.name(), BUSINESS_PRICING_APPROVAL_MEETING_REPORT.name());
//    }
//
...
//                YIELD_REVIEW_REPORT.name(), MEETING_REVIEW_REPORT.name(), MEETING_REVIEW_RECORD.name(), DIRECTOR_MEETING_REPORT.name(), OTHER.name(), PROFESSIONAL_REVIEW_COMMENTS.name());
//    }
