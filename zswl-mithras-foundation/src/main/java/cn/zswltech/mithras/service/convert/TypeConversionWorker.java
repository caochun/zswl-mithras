...
public class TypeConversionWorker {
    ...
    public List<ClientInfo> jsonStringToClientInfoList(String jsonStr) {
        ...
        if (CollectionUtil.isNotEmpty(res)) {
            // 查询最新信息
            Map<Long, String> map = SpringUtil.getBean(ClientNameResolver.class).clientId2Name(res.stream().map(ClientInfo::getClientId).collect(Collectors.toList()));
    ...
    public List<ProjEstablishPersonInfo> jsonStringToPersonInfoList(String jsonStr) {
        ...
        if (CollectionUtil.isNotEmpty(res)) {
            // 查询最新信息
            Map<Long, String> map = SpringUtil.getBean(ClientNameResolver.class).clientId2Name(res.stream().map(ProjEstablishPersonInfo::getClientId).collect(Collectors.toList()));
