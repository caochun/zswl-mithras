...
public class DataAuthCheckAdvice {
    ...
    @Pointcut("@annotation(cn.zswltech.mithras.foundation.auth.aop.DataAuthCheck)")
    ...
    public void doBefore(JoinPoint joinPoint, DataAuthCheck dataAuthCheck) {
        ...
        DataAuthBusinessModule businessModule = dataAuthBusinessModuleResolver.resolve(dataAuthCheck.businessModule());
        Class<? extends BaseMapper>[] classes = dataAuthCheck.mapperClass();
        for (Class<? extends BaseMapper> aClass : classes) {
            if (key == null) {
                dataAuthChecker.check(businessModule, aClass, null, args);
            } else {
                if (key instanceof Long) {
                    dataAuthChecker.check(businessModule, aClass, (Long) key, args);
                } else if (key instanceof String) {
                    String s = (String) key;
                    if (!NumberUtil.isLong(s)) {
                        throw new AuthCheckException("数据主Key类型错误");
                    }
                    dataAuthChecker.check(businessModule, aClass, Long.parseLong(s), args);
                } else if (key instanceof Collection) {
                    dataAuthChecker.checkBatch(businessModule, aClass, (Collection) key, args);
                } else {
