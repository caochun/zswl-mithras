...
public class CollectionPenaltyInterestJobServiceImpl implements CollectionPenaltyInterestJobService {
    ...
    @Resource
    private CollectionPenaltyInterestJobSupportPort supportPort;
    ...
    public void penaltyInterestJobHandler() {
        ...
        for (CollectionBaseInfo info : list) {
            ...
            if (!contractPenaltyInterestRateMap.containsKey(info.getContractId())) {
                contractPenaltyInterestRateMap.put(info.getContractId(), supportPort.getDailyRate(info.getContractId()));
            }
            ...
            long dayAmount = mithrasInteger2BigDecimal(rate).multiply(mithrasLong2BigDecimal(overdueAmount)).multiply(BigDecimal.valueOf(100L)).setScale(0, BigDecimal.ROUND_HALF_UP).longValue();
            long last = LongUtil.null2zero(info.getPenaltyInterest()) + dayAmount;
            recordInfo.setOverdueAmount(overdueAmount);
            // 保留两位小数到分
            recordInfo.setDayPenaltyInterest(mithrasLongDecimalTwo(dayAmount));
            // 保留两位小数到分
            recordInfo.setLastPenaltyInterest(mithrasLongDecimalTwo(last));
            ...
            if (overdueRecordInfo == null) {
                ...
                for (LocalDate date = info.getPlanCollectionDate().plusDays(1); date.isBefore(LocalDate.now()); date = date.plusDays(1)) {
                    ...
                    sunLast += last;
                    collectionOverdueRecordInfo.setLastPenaltyInterest(mithrasLongDecimalTwo(sunLast));
            ...
            // 保留两位小数到分
            info.setPenaltyInterest(mithrasLongDecimalTwo(last));
    ...
    **/
    ...
    public void penaltyInterestJobHandler2(LocalDate localDate) {
        try {
            ...
            //记录昨天逾期金额
            supportPort.overdueSnapshot(localDate);
            //查询某一天的逾期记录
            List<CollectionOverdueHistory> historyByDay = supportPort.getHistoryByDay(localDate);
            //维护税率
            ...
            Map<Long, Integer> dailyRateBatchMap = supportPort.getDailyRateBatch(new ArrayList<>(contractIds));
            ...
            //获取每笔租金的最后一次罚息记录情况
            List<CollectionOverdueRecordInfo> lastRecordByCollectionId = supportPort.getLastRecordByCollectionId(new ArrayList<>(collectIds));
            ...
            for (CollectionOverdueHistory info : historyByDay) {
                ...
                BigDecimal dayAmount = mithrasInteger2BigDecimal(rate).multiply(mithrasLong2BigDecimal(overdueAmount)).multiply(BigDecimal.valueOf(100L)).setScale(10, BigDecimal.ROUND_HALF_UP);
                ...
                List<CollectionOverdueHistory> historyByDay1 = supportPort.getHistoryByDay(collectionBaseInfo.getId());
                ...
                if (overdueRecordInfo == null && collectionBaseInfo.getPlanCollectionDate().isBefore(localDate)) {
                    ...
                    for (LocalDate date = collectionBaseInfo.getPlanCollectionDate().plusDays(1); date.isBefore(LocalDate.now().minusDays(1)); date =
                        ...
                        nowAmount = mithrasInteger2BigDecimal(rate).multiply(mithrasLong2BigDecimal(nowDueAmount)).multiply(BigDecimal.valueOf(100L)).setScale(10, BigDecimal.ROUND_HALF_UP);
                        collectionOverdueRecordInfo = BeanUtil.copyProperties(recordInfo, CollectionOverdueRecordInfo.class);
                        collectionOverdueRecordInfo.setRecordDate(date);
                    ...
                    if (ObjectUtil.isNotEmpty(lastOverdueRecordInfo)) {
                        ...
                        for (LocalDate date = lastOverdueRecordInfo.getRecordDate(); date.isBefore(LocalDate.now()); date =
                            ...
                            nowAmount = mithrasInteger2BigDecimal(rate).multiply(mithrasLong2BigDecimal(nowDueAmount)).multiply(BigDecimal.valueOf(100L)).setScale(0, BigDecimal.ROUND_HALF_UP);
                            collectionOverdueRecordInfo = BeanUtil.copyProperties(recordInfo, CollectionOverdueRecordInfo.class);
                            collectionOverdueRecordInfo.setRecordDate(date);
                            collectionOverdueRecordInfo.setOverdueAmount(nowDueAmount);
                            collectionOverdueRecordInfo.setDayPenaltyInterest(nowAmount.longValue());
                            sunLast = sunLast.add(nowAmount);
                            collectionOverdueRecordInfo.setLastPenaltyInterest(mithrasLongDecimalTwo(sunLast.longValue()));
                            addCollectionOverdueRecordInfo.add(collectionOverdueRecordInfo);
                        }
            ...
            if (CollUtil.isNotEmpty(updateCollectionBaseInfo)) {
                supportPort.updateCollectionBaseInfoBatchById(updateCollectionBaseInfo);
            }
            if (CollUtil.isNotEmpty(addCollectionOverdueRecordInfo)) {
                supportPort.saveOverdueRecordBatch(addCollectionOverdueRecordInfo);
            }
            log.info("PenaltyInterestJob, end.");
        } catch (Exception e) {
    ...
    public void penaltyInterestJobHandler3(LocalDate localDate){
        try {
            ...
            supportPort.doRerunPenaltyInterest(collectionIds);
            //维护核销状态
            supportPort.modifyWriteOffStatus(collectionIds);
    ...
    private static BigDecimal mithrasInteger2BigDecimal(Integer value) {
        if (value == null) {
            return null;
        }
        return BigDecimal.valueOf(value).divide(BigDecimal.valueOf(10000L));
