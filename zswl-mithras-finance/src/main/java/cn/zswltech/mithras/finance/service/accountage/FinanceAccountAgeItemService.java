...
import cn.zswltech.mithras.finance.util.SnowflakeIdGenerator;
...
public class FinanceAccountAgeItemService extends ServiceImpl<FinanceAccountAgeItemMapper, FinanceAccountAgeItem> {
    ...
    @Resource
    private FinanceAccountAgeSupportPort financeAccountAgeSupportPort;
    ...
    public void accountAgeInit(Long accountAgeId) {
        ...
        //查询截止日期前的租金
        List<CollectionBaseInfo> allCollectionBaseList = collectionBaseInfoMapper.selectList(Wrappers.<CollectionBaseInfo>lambdaQuery()
    ...
    public List<FinanceAccountAgeItem> calculateFinanceAccountAge(List<CollectionBaseInfo> allCollectionBaseList, Long accountAgeId) {
        ...
        //获取不同时间的租金的未核销金额
        Map<Long, Long> newAddAmountMap = financeAccountAgeSupportPort.sumRemainingPrincipalByCollection(allCollectionBaseList.stream().filter(baseInfo -> !receiptIdSet.contains(baseInfo.getReceiptId())).map(CollectionBaseInfo::getId).collect(Collectors.toList()), financeAccountAgeBaseInfo.getDeadline());
        Map<Long, Long> oldAmountMap = financeAccountAgeSupportPort.sumRemainingPrincipalByCollection(allCollectionBaseList.stream().filter(baseInfo -> receiptIdSet.contains(baseInfo.getReceiptId())).map(CollectionBaseInfo::getId).collect(Collectors.toList()), financeAccountAgeBaseInfo.getDeadline());
        stopWatch.stop();
        stopWatch.start("构建");
        //开始构建
    ...
    public void regeneration(@Valid FinanceAccountAgeItemRemoveREQ req){
        ...
        this.check(originalInfos.get(0).getAccountAgeId());
        SpringContextHolder.getBean(FinanceAccountAgeItemService.class).calculateFinanceAccountAge(financeAccountAgeSupportPort.listCollections(originalInfos.stream().map(FinanceAccountAgeItem::getCollectionId).collect(Collectors.toSet())), originalInfos.get(0).getAccountAgeId());
    }
   ...
   public void sendRemote(@Valid FinanceAccountAgeItemRemoveREQ req) {
       ...
       originalInfos.forEach(e -> {
           ...
           if (sendInfos.size() == 100) {
               ...
               financeAccountAgeSupportPort.sendAccountAge(cq2AccountAgeAddREQS);
               sendInfos.clear();
           }
       });
