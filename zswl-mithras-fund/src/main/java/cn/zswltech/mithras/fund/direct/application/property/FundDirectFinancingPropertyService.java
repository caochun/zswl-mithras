package cn.zswltech.mithras.fund.direct.application.property;
...
import cn.zswltech.mithras.fund.direct.excel.exporter.PropertyExcelManagerExporter;
import cn.zswltech.mithras.fund.direct.excel.model.PropertyExcelModel;
...
import cn.zswltech.mithras.fund.direct.mapper.PropertyMapper;
...
public class FundDirectFinancingPropertyService
    ...
    public PageR<FundFinancingPropertyRSP> list(FundFinancingPropertyListREQ req) {
        try{
            ...
            //查询直融编号
            FundDirectFinancingBaseInfo directBaseInfo = fundDirectFinancingBaseInfoMapper.selectById(financingId);
            if(directBaseInfo != null){
                financingCode = directBaseInfo.getFinancingCode();
            }
            //查询间融编号
            FundFinancingBaseInfo indirectBaseInfo = fundFinancingBaseInfoMapper.selectById(financingId);
            if(indirectBaseInfo != null){
                financingCode = indirectBaseInfo.getFinancingCode();
            }
            //直融间融都不是，返回空列表
            ...
            for(FundFinancingPropertyRSP fundFinancingPropertyRSP:rspList){
                ...
                if(fundFinancingPropertyRSP.getFinancingCode().startsWith("ZR")){
                    ...
                    FundDirectFinancingBaseInfo baseInfo =
                            fundDirectFinancingBaseInfoMapper.selectById(financingId);
                ...
                if(fundFinancingPropertyRSP.getFinancingCode().startsWith("DK")){
                    ...
                    if(!fundFinancingCreditRefs.isEmpty()){
                        ...
                        //取融资金额
                        FundFinancingBaseInfo baseInfo = fundFinancingBaseInfoMapper.selectById(financingId);
