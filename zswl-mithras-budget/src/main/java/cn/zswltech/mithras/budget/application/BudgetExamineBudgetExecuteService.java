 ...
 */
@Service
public class BudgetExamineBudgetExecuteService extends ServiceImpl<BudgetExamineBudgetExecuteMapper, BudgetExamineBudgetExecute> implements BudgetExamineBudgetExecuteApplicationService {
    @Resource
    ...
    public void add(BudgetExamineBudgetExecuteAddREQ req) {
        ...
        for (OrgDO org : SpringUtil.getBean(SysUserService.class).listBizDept()) {
            ...
            if (projectCount != 0) {
                ...
                expenseEfficiencyExecute.setTotalYear(BudgetFinancialUtil.mithrasLongDecimalTwo(b.longValue()));
            } else {
        ...
        for (BudgetExamineBudgetExecute initExecute : initList) {
            ...
            // 进度预算目标 = 全年预算目标/12*月份
            initExecute.setProgressBudgetTarget(BudgetFinancialUtil.mithrasLongDecimalTwo(BigDecimal.valueOf(initExecute.getAnnualBudgetTarget()).divide(BigDecimal.valueOf(12), 20, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(req.getBudgetExamineMonth())).longValue()));
            // 进度预算完成率（%） = 本年累计/进度预算目标
            if (!Objects.equals(initExecute.getProgressBudgetTarget(), 0L) && Objects.nonNull(initExecute.getTotalYear())) {
                initExecute.setProgressBudgetCompletionRate(BudgetFinancialUtil.mithrasLongDecimalTwo(BigDecimal.valueOf(initExecute.getTotalYear()).divide(BigDecimal.valueOf(initExecute.getProgressBudgetTarget()), 20, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(1000000)).longValue()));
            }
            // 全年预算完成率（%）= 本年累计/全年预算目标
            if (!Objects.equals(initExecute.getAnnualBudgetTarget(), 0L) && Objects.nonNull(initExecute.getTotalYear())) {
                initExecute.setAnnualBudgetCompletionRate(BudgetFinancialUtil.mithrasLongDecimalTwo(BigDecimal.valueOf(initExecute.getTotalYear()).divide(BigDecimal.valueOf(initExecute.getAnnualBudgetTarget()), 20, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(1000000)).longValue()));
            }
        }
    ...
    private Long division(Long numerator, Long denominator, boolean needMinusOne) {
        ...
        if (needMinusOne) {
            ...
            return BudgetFinancialUtil.mithrasLongDecimalTwo(b.longValue());
        }
        return BudgetFinancialUtil.mithrasLongDecimalTwo(b.longValue());
    }
