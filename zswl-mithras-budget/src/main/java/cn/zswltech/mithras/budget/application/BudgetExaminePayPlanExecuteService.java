...
import cn.zswltech.mithras.budget.domain.enums.BudgetPlanPayFundPlanEnum;
...
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.BudgetExaminePayPlanExecuteMapper;
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.BudgetPlanMapper;
import cn.zswltech.mithras.budget.infrastructure.persistence.mapper.BudgetPlanPayDetailMapper;
...
public class BudgetExaminePayPlanExecuteService extends ServiceImpl<BudgetExaminePayPlanExecuteMapper, BudgetExaminePayPlanExecute> implements BudgetExaminePayPlanExecuteApplicationService {
    ...
    public void add(BudgetExaminePayPlanExecuteAddREQ req) {
        ...
        //本月周报计划
        List<BudgetPlanPayWeeklyReport> monthWeeks = getMonthWeek(budgetExamine.getExamineYear(), budgetExamine.getExamineMonth());
        List<BudgetPlanPayWeeklyReportDetail> monthDetailWeeks = new ArrayList<>();
        if (ObjectUtil.isNotEmpty(monthWeeks)) {
            monthDetailWeeks = budgetPlanPayWeeklyReportDetailMapper.selectList(Wrappers.<BudgetPlanPayWeeklyReportDetail>lambdaQuery()
        ...
        for (OrgDO org : orgList) {
            ...
            if (thisMonthPayment < LongUtil.null2zero(fundsPlanAmount)) {
                ...
                if (fundsPlanAmount == 0) {
                    ...
                    budgetExaminePayPlanExecute.setDeviationDegreePlan(BudgetFinancialUtil.mithrasIntegerDecimalTwo(b.intValue()));
                }
            ...
            if (thisMonthReviews.isEmpty()) {
                budgetExaminePayPlanExecute.setProjectAccuracy(BudgetFinancialUtil.mithrasIntegerDecimalTwo(0));
            } else {
                ...
                budgetExaminePayPlanExecute.setProjectAccuracy(BudgetFinancialUtil.mithrasIntegerDecimalTwo(projectAccuracyBD.intValue()));
            }
    ...
    private Map<Long, Long> getDept2FundsPlanAmount(BudgetPlan budgetPlanMonth, BudgetPlan budgetPlanAdjust) {
        ...
        List<BudgetPlanPayDetail> planPayDetails = budgetPlanPayDetailMapper.selectList(
        ...
        List<BudgetPlanPayDetail> budgetPlanAdjustDetail = budgetPlanPayDetailMapper.selectList(
                Wrappers.<BudgetPlanPayDetail>lambdaQuery()
        ...
        for (OrgDO org : orgList) {
            ...
            dept2FundsPlanAmountMap.put(org.getId(), BudgetFinancialUtil.mithrasLongDecimalTwo(b.longValue()));
        }
        return dept2FundsPlanAmountMap;
    }
    ...
    private Map<Long, Long> getDept2BusinessPlanAmount(BudgetPlan budgetPlan, BudgetPlan budgetPlanAdjust) {
        ...
        List<BudgetPlanPayDetail> planPayAdjustDetails = budgetPlanPayDetailMapper.selectList(
    ...
    private List<BudgetPlanPayWeeklyReport> getMonthWeek(Integer planYear, Integer planMonth) {
        LocalDate start = LocalDate.of(planYear, planMonth, 1);
        LocalDate end = start.plusMonths(1);
        return budgetPlanPayWeeklyReportMapper.selectList(Wrappers.<BudgetPlanPayWeeklyReport>lambdaQuery()
