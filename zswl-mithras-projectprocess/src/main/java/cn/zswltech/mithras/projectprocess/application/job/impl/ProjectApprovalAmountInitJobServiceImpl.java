...
public class ProjectApprovalAmountInitJobServiceImpl implements ProjectApprovalAmountInitJobService {
    ...
    @Autowired
    private ProjReviewAocPriceMapper aocPriceMapper;
    @Autowired
    private ProjReviewAocPriceLibService aocPriceLibService;
    @Autowired
    private ProjReviewFactoringPriceMapper factoringPriceMapper;
    @Autowired
    ...
    public void projectApprovalAmountInitJob() {
        ...
        try {
            ...
            List<ProjReviewLeasePrice> projReviewLeasePrices = new ArrayList<>();
            if (CollUtil.isNotEmpty(projectIds)) {
                projReviewLeasePrices = leasePriceMapper.selectList(Wrappers.<ProjReviewLeasePrice>lambdaQuery()
                        .in(ProjReviewLeasePrice::getProjectId, projectIds));
            }
            ...
            List<ProjReviewFactoringPrice> projReviewFactoringPrices = new ArrayList<>();
            if (CollUtil.isNotEmpty(projectIds)) {
                projReviewFactoringPrices = factoringPriceMapper.selectList(Wrappers.<ProjReviewFactoringPrice>lambdaQuery()
                        .in(ProjReviewFactoringPrice::getProjectId, projectIds));
            }
            ...
            List<ProjReviewAocPrice> projReviewAocPrices = new ArrayList<>();
            if (CollUtil.isNotEmpty(projectIds)) {
                projReviewAocPrices = aocPriceMapper.selectList(Wrappers.<ProjReviewAocPrice>lambdaQuery()
                        .in(ProjReviewAocPrice::getProjectId, projectIds));
            }
            ...
            if (CollUtil.isNotEmpty(leasePrices)) {
                leasePrices.forEach(leasePriceMapper::updateById);
            }
            if (CollUtil.isNotEmpty(factoringPrices)) {
                factoringPrices.forEach(factoringPriceMapper::updateById);
            }
            if (CollUtil.isNotEmpty(aocPrices)) {
                aocPrices.forEach(aocPriceMapper::updateById);
            }
        } catch (Exception e) {
