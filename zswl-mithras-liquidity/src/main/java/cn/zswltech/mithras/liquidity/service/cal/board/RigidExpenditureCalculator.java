 ...
 */
@Component
public class RigidExpenditureCalculator extends AbstractLiquidityCalculator<LiquidityBoardCalculatorBo> {
    ...
    public void calculate(Object obj, LiquidityBoardCalculatorBo bo) {
        ...
        if(CollectionUtil.isNotEmpty(accountBalanceMap)){
            result = accountBalanceMap.values().stream().map(m -> {
                return LongUtil.null2zeroBigDecimal(m.getMustExpenseAmount());
