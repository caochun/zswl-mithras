 ...
 */
@Component
public class ExpectedRentRecoveryCalculator extends AbstractLiquidityCalculator<LiquidityBoardCalculatorBo> {
    ...
    public void calculate(Object obj, LiquidityBoardCalculatorBo bo) {
        ...
        if(CollectionUtil.isNotEmpty(accountBalanceMap)){
            result = accountBalanceMap.values().stream().map(m -> {
                return LongUtil.null2zeroBigDecimal(m.getRentReflowAmount());
