...
(
    ...
    `select_status`            varchar(1)           DEFAULT NULL COMMENT '查询状态',
...
(
    ...
    `update_by`         bigint(20)                   DEFAULT NULL COMMENT '更新人',
    `create_time`                      datetime       DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`                      datetime       DEFAULT CURRENT_TIMdashboard_visit_infoESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`                          tinyint(4) NOT NULL DEFAULT '0' COMMENT '逻辑删除，0-未删除，1-已删除',
    PRIMARY KEY (`id`)
...
VALUES ('creditReportManager', '征信管理', 1, 80, null, null, 'icon-zhengxinbaosong', null, null);

update bifrost_custom_tree set parent_id = -999999
where code = 'creditReport';

INSERT INTO bifrost_custom_tree (code, name, flag, sort_no, parent_id, en_name, icon, path, lang_env)
VALUES ('creditReportQuery', '征信查询', 1, 80, -999999, null, null, null, null);
...
from bifrost_custom_tree a,
     bifrost_menu b
where a.code = 'creditReportQuery'
  and b.code = 'CreditManageSearch';

update bifrost_menu set path = '/creditManage/creditTable/wait' where id  = 248;
update bifrost_menu set path = '/creditManage/creditTable/finish' where id  = 258;


-- delete from bifrost_org_menu_function where menu_id in (248,258);

-- delete from bifrost_role_menu_function where menu_id in (248,258);
