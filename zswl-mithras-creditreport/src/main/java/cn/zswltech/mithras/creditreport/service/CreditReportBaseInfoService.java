...
import cn.zswltech.mithras.projectprocess.service.projestablish.ProjEstablishTradeStructureService;
import cn.zswltech.mithras.projectprocess.service.projreview.ProjReviewTradeStructureService;
...
public class CreditReportBaseInfoService extends ServiceImpl<CreditReportBaseInfoMapper, CreditReportBaseInfo> implements CreditReportQueryService, CreditReportResultApplicationService {
    ...
    public PageR<CreditReportListDTO> list(CreditReportListREQ req) {
        ...
        if (CollectionUtil.isEmpty(projmanagerList) && CollectionUtil.isEmpty(businessheadList) && CollectionUtil.isEmpty(leaderinchargeList)) {
            ...
            if (CollectionUtil.isNotEmpty(projmanagerList)) {
                Set<Long> projmanagerTargetClientIds = creditReportClientSupportPort.findTargetClientIdsByUserId(currentUserId);
            ...
            if (CollectionUtil.isNotEmpty(businessheadList)) {
                ...
                Set<Long> businessheadTargetClientIds = creditReportClientSupportPort.findTargetClientIdsByDeptIds(deptIds);
            ...
            if (CollectionUtil.isNotEmpty(leaderinchargeList)) {
                ...
                Set<Long> leaderinchargeTargetClientIds = creditReportClientSupportPort.findTargetClientIdsByDeptIds(deptIds);
    ...
    public List<CreditReportSubmitDTO> submit(CreditReportSubmitCmd cmd) {
        ...
        //校验企业资料是否上传
        Map<String, List<String>> map = checkEnterprise(clientItems, CreditReportMaterialTypeEnum.ENTERPRISE_CREDIT_REPORT.name(), CreditReportBusinessModule.CREDIT_REPORT_SELECT.name());
        if (map.size() > 0) {
