 ...
 * 期初余额（元）
 *
 *  账户明细表中上一日全部账户结余（实际）之和
 *  无结余（实际）则取结余（预估）
 * @author chenyifei
 * @since 2024/12/20
 */
@Component
public class InitialBalanceCalculator extends AbstractLiquidityCalculator<LiquidityBoardCalculatorBo> {

    @Override
    public void calculate(Object obj, LiquidityBoardCalculatorBo bo) {
        BigDecimal result = BigDecimal.ZERO;
        LocalDate calculateDate = bo.getQueryDate().minusDays(1);
        Map<Long, AccountBalanceBaseInfo> accountBalanceMap = LiquidityIndicatorBoardHolder.ACCOUNT_BALANCE_BASE_INFO.get(calculateDate);
