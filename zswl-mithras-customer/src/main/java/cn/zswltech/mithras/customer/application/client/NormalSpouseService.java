...
import cn.zswltech.mithras.customer.model.client.NormalSpouse;
import cn.zswltech.mithras.customer.model.client.NormalSpouseLib;
...
public class NormalSpouseService implements ClientDataSaveCheckInterface<NormalSpouse> {
    ...
    public PageR<NormalSpouseListRSP> list(NormalSpouseListREQ req) {
        ...
        Set<String> certNumbers = resPage.getList().stream().map(NormalSpouseListRSP::getCertNumber).collect(Collectors.toSet());
        Map<String, Long> idMap = certNumbers.isEmpty() ? Collections.emptyMap() : clientMapper.selectList(Wrappers.<Client>lambdaQuery()
                .eq(Client::getClientType, ClientType.NORMAL.name())
                .in(Client::getCertNumber, certNumbers))
