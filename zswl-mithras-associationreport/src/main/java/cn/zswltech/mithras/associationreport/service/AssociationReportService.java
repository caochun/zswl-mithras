...
import cn.zswltech.mithras.associationreport.storedata.DeleteDataSelector;
...
import cn.zswltech.mithras.associationreport.mapper.AssociationReportSortMapper;
...
public class AssociationReportService extends ServiceImpl<AssociationReportMapper, AssociationReport> implements AssociationReportQueryService, AssociationReportApplicationService, AssociationReportJobService {
    ...
    public String getTemplateFileUrl(String reportCategoryCode) {
        ...
        return templateFilePort.getTemplateFileUrl(fileTemplateType, fileTemplateName);
    }
...
/*        if (Objects.equals(associationReport.getReportPeriodCategory(), AssociationReportPeriodCategoryEnum.REALTIME.name())) {
    ...
    public void createPushFlow(List<AssociationReport> reportList) {
        ...
        String reportCategoryNames = StrUtil.join("/", reportList.stream()
                .sorted(Comparator.comparing(AssociationReport::getReportCategoryCode))
                .map(AssociationReport::getReportCategoryName)
                .collect(Collectors.toSet()));
        workflowPort.startPushFlow(associationReportApply.getId(), reportCategoryNames);
    }
